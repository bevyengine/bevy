...
- [Remove dependencies from bevy_tasks' README][9881]
...
- [Fix incorrect doc comment for the set method of `ContentSize`][9345]
...
- [Fix tonemapping test pattern][10092]
...
- [Replace all usages of texture_descriptor.size.* with the helper methods][10227]
...
- [Add methods for calculating the size and position of UI nodes][7930]
...
- [Only trigger state transitions if `next_state != old_state`][8359]
...
- [Windowing: Modified the `WindowDescriptor`’s `Default` impl.][6664]
...
- [Render: Interpret glTF colors as linear instead of sRGB][6828]
...
- [Asset: Fix asset_debug_server hang. There should be at most one ThreadExecutor][7825]
...
- [Remove unnecessary branches/panics from Query accesses][6461]
...
- [Visibility Inheritance, universal `ComputedVisibility`, and `RenderLayers` support][5310]
...
- [Add `ExactSizeIterator` implementation for `QueryCombinationIter`][5148]
...
- [Copy `TaskPool` resources to sub-Apps][4792]
...
- [Use u32 over usize for `ComponentSparseSet` indices][4723]
...
- [Move the CoreStage::Startup to a separate StartupSchedule label][2434]
...
- [update archetypes for run criteria][2177]
...
- [Prevent double panic in the Drop of TaskPoolInner][1064]
...
- [Custom rodio source for audio.][145]
  - `AudioOutput` is now able to play anything `Decodable`.
