...
///
/// A [`bevy_render::Mesh`] can be converted to a [`MeshletMesh`] using `MeshletMesh::from_mesh` when the `meshopt` cargo feature is enabled.
...
pub struct MeshletMesh {
    /// The total amount of indices in the overall mesh (sum of meshlet index_counts).
