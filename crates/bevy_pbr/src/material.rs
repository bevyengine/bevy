...
/// dispatch. Any custom WESL entry (vertex or fragment) must declare
...
/// (the default `mesh.wesl` vertex entry threads `view_index` itself);
...
///
/// See `pbr.wesl` for the canonical fragment-entry pattern,
/// `mesh.wesl` for the vertex-entry pattern, and
/// `mesh_view_bindings.wesl` for a paste-ready snippet.
///
