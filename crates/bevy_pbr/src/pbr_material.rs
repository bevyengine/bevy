...
pub struct StandardMaterial {
    ...
    ///
    /// The default emissive color is [`LinearRgba::BLACK`], which doesn't add anything to the
    /// material color.
    ///
    ...
    /// In opposition, a value of `1.0` means the emissive color is multiplied by the camera
    /// exposure.
    ///
    /// Defaults to `0.0`
    ...
    /// dielectric objects (e.g. paper, leaves, some fabrics) or thicker volumetric materials
    ...
    /// allows occluded objects to be seen through the material, with distortion and blur
    /// effects.
    ///
    ...
    ///   [`StandardMaterial::diffuse_transmission`] instead, for a much less expensive effect.
...
impl StandardMaterial {
    ...
    ///
    /// Multiplying this with another Affine2 returns transformation with horizontally flipped
    /// texture coords
    ...
    ///
    /// Multiplying this with another Affine2 returns transformation with vertically flipped texture
    /// coords
...
pub struct StandardMaterialUniform {
    ...
    // Might be used in the future for exposure correction in HDR
    pub emissive: Vec4,
    /// Color white light takes after traveling through the attenuation distance underneath the
    /// material surface
