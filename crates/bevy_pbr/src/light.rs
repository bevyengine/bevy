...
pub fn add_clusters(
    ...
    windows: Res<Windows>,
    cameras: Query<(Entity, &Camera, Option<&ClusterConfig>), Without<Clusters>>,
) {
    for (entity, camera, config) in cameras.iter() {
...
fn update_clusters(
    ...
    *clusters = Clusters::from_screen_size_and_dimensions(screen_size, cluster_dimensions);
