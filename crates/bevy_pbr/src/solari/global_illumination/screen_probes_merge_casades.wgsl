...
fn merge_screen_probe_cascades(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let br_probe_weight = (1.0 - r.x) * r.y;
...
fn sample_upper_probe(tl_pixel_id: vec2<u32>) -> vec4<f32> {
    ...
    let tr_direction_sample = textureLoad(screen_probes_a, tl_pixel_id + vec2(0u, 1u), lower_cascade + 1u);
    let bl_direction_sample = textureLoad(screen_probes_a, tl_pixel_id + vec2(1u, 0u), lower_cascade + 1u);
