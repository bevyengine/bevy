...
fn fetch_point_shadow(light_id: u32, frag_position: vec4<f32>, surface_normal: vec3<f32>) -> f32 {
    ...
    // NOTE: Cubemap sampling requires a left-handed coordinate system. From this point on, we
    // switch to left-handed coordinates by flipping the sign of the z coordinate.

    // similar largest-absolute-axis trick as above, but now with the offset fragment position
    let offset_light_to_fragment_lh = flip_z.xyz * (offset_position.xyz - light.position_radius.xyz);
    let abs_offset_light_to_fragment_lh = abs(offset_light_to_fragment_lh);
    let major_axis_magnitude = max(
        abs_offset_light_to_fragment_lh.x,
        max(
            abs_offset_light_to_fragment_lh.y,
            abs_offset_light_to_fragment_lh.z
        )
    );

    // NOTE: These simplifications come from multiplying (left-handed projection and z):
