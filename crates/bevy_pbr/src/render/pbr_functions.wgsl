...
#import bevy_pbr::{
    pbr_types, pbr_bindings, mesh_view_bindings, mesh_view_types,
    lighting, clustered_forward, shadows, fog, ambient,
    mesh_bindings::mesh,
...
fn calculate_view(
    ...
    if is_orthographic {
        ...
        V = normalize(vec3<f32>(mesh_view_bindings::view.view_proj[0].z, mesh_view_bindings::view.view_proj[1].z, mesh_view_bindings::view.view_proj[2].z));
    } else {
        // Only valid for a perpective projection
        V = normalize(mesh_view_bindings::view.world_position.xyz - world_position.xyz);
    }
    return V;
}
...
fn pbr(
    ...
    let view_z = dot(vec4<f32>(
        mesh_view_bindings::view.inverse_view[0].z,
        mesh_view_bindings::view.inverse_view[1].z,
        mesh_view_bindings::view.inverse_view[2].z,
        mesh_view_bindings::view.inverse_view[3].z
    ), in.world_position);
    let cluster_index = clustered_forward::fragment_cluster_index(in.frag_coord.xy, view_z, in.is_orthographic);
    let offset_and_counts = clustered_forward::unpack_offset_and_counts(cluster_index);

    // Point lights (direct)
    for (var i: u32 = offset_and_counts[0]; i < offset_and_counts[0] + offset_and_counts[1]; i = i + 1u) {
        let light_id = clustered_forward::get_light_id(i);
        var shadow: f32 = 1.0;
        if ((in.flags & MESH_FLAGS_SHADOW_RECEIVER_BIT) != 0u
                && (mesh_view_bindings::point_lights.data[light_id].flags & mesh_view_types::POINT_LIGHT_FLAGS_SHADOWS_ENABLED_BIT) != 0u) {
    ...
    for (var i: u32 = 0u; i < n_directional_lights; i = i + 1u) {
        var shadow: f32 = 1.0;
        if ((in.flags & MESH_FLAGS_SHADOW_RECEIVER_BIT) != 0u
                && (mesh_view_bindings::lights.directional_lights[i].flags & mesh_view_types::DIRECTIONAL_LIGHT_FLAGS_SHADOWS_ENABLED_BIT) != 0u) {
    ...
    output_color = clustered_forward::cluster_debug_visualization(
        output_color,
        view_z,
        in.is_orthographic,
        offset_and_counts,
