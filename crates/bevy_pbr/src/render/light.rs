...
pub fn prepare_lights(
    ...
    // - then by entity as a stable key to ensure that a consistent set of lights are chosen if the
    //   light count limit is exceeded.
    // - because entities are unique, we can use `sort_unstable_by_key` and still end up with a
    //   stable order.
        ...
        .filter_map(|sorted_camera| views.get(sorted_camera.entity).ok())
    {
            ...
            .take(MAX_DIRECTIONAL_LIGHTS)
        {
                ...
                iter.zip(light_view_entities.iter().copied()).enumerate()
            {
                ...
                // Subsequent views with the same light entity will **NOT** reuse the same shadow
                // map (Because the cascades are unique to each view)
