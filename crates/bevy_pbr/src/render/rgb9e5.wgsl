...
const RGB9E5_MANTISSA_BITS_U      = 9u;
...
const MAX_RGB9E5_MANTISSA_U       = 511u;
...
fn vec3_to_rgb9e5_(rgb_in: vec3<f32>) -> u32 {
    ...
    let max_rgb = max(rgb.r, max(rgb.g, rgb.b));
    var exp_shared = max(-RGB9E5_EXP_BIAS - 1, floor_log2_(max_rgb)) + 1 + RGB9E5_EXP_BIAS;
    var denom = exp2(f32(exp_shared - RGB9E5_EXP_BIAS - RGB9E5_MANTISSA_BITS));

    let max_m = i32(floor(max_rgb / denom + 0.5));
    if (max_m == RGB9E5_MANTISSA_VALUES) {
        denom *= 2.0;
        exp_shared += 1;
    }
...
fn rgb9e5_to_vec3_(v: u32) -> vec3<f32> {
    ...
    return vec3(
        f32(extract_bits(v, 0u, RGB9E5_MANTISSA_BITS_U)),
        f32(extract_bits(v, 9u, RGB9E5_MANTISSA_BITS_U)),
        f32(extract_bits(v, 18u, RGB9E5_MANTISSA_BITS_U))
    ) * scale;
}
