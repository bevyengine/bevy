...
fn point_light(
    ...
    if ((*light).flags & POINT_LIGHT_FLAGS_MONOCHROMATIC_BIT) != 0u {
        ...
        let response = monochromatic_response(color_times_NdotL, (*light).color_inverse_square_range.rgb * rangeAttenuation * texture_sample);
...
fn monochromatic_response(base: vec3<f32>, light: vec3<f32>) -> f32 {
