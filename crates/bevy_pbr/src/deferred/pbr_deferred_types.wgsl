...
const U12_MAX_F = 4095.0;
const U16_MAX_F = 65535.0;
const U20_MAX_F = 1048575.0;
...
fn pack_24bit_normal_and_flags(octahedral_normal: vec2<f32>, flags: u32) -> u32 {
    let unorm1 = u32(saturate(octahedral_normal.x) * U12_MAX_F + 0.5);
    let unorm2 = u32(saturate(octahedral_normal.y) * U12_MAX_F + 0.5);
...
fn unpack_24bit_normal(packed: u32) -> vec2<f32> {
    ...
    return vec2(f32(unorm1) / U12_MAX_F, f32(unorm2) / U12_MAX_F);
}
...
fn pack_unorm3x4_plus_unorm_20_(v: vec4<f32>) -> u32 {
    ...
    let bg = u32(saturate(v.w) * U20_MAX_F + 0.5);
