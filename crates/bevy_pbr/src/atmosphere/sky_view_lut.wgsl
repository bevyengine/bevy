...
fn main(in: FullscreenVertexOutput) -> @location(0) vec4<f32> {
    ...
    for (var step_i: u32 = 0u; step_i < settings.sky_view_lut_samples; step_i++) {
        ...
        var local_illuminance = sample_local_illuminance(transmittance_to_sample, view_dir, altitude);
...
fn sample_local_illuminance(transmittance_to_sample: vec3<f32>, view_dir: vec3<f32>, altitude: f32) -> vec3<f32> {
