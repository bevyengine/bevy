...
impl MeshBuilder for Capsule3dMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let mut vs: Vec<Vec3> = alloc::vec![Vec3::ZERO; vert_len];
        let mut vts: Vec<Vec2> = alloc::vec![Vec2::ZERO; vert_len];
        let mut vns: Vec<Vec3> = alloc::vec![Vec3::ZERO; vert_len];
        ...
        let mut theta_cartesian: Vec<Vec2> = alloc::vec![Vec2::ZERO; longitudes as usize];
        let mut rho_theta_cartesian: Vec<Vec2> = alloc::vec![Vec2::ZERO; longitudes as usize];
        let mut s_texture_cache: Vec<f32> = alloc::vec![0.0; lonsp1 as usize];
        ...
        let mut tris: Vec<u32> = alloc::vec![0; fs_len as usize];
