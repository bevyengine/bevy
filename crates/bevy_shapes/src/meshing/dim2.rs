...
impl MeshBuilder for CircularSectorMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; resolution + 1];
...
impl MeshBuilder for ConvexPolygonMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; len];
...
impl MeshBuilder for EllipseMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; resolution];
...
impl MeshBuilder for AnnulusMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; num_vertices];
...
impl MeshBuilder for RhombusMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let uvs = alloc::vec![[1.0, 0.5], [0.5, 0.0], [0.0, 0.5], [0.5, 1.0]];
        let indices = Indices::U32(alloc::vec![2, 0, 1, 2, 3, 0]);
...
impl MeshBuilder for Triangle2dMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; 3];

        let uvs: Vec<_> = crate::meshing::triangle3d::uv_coords(&Triangle3d::new(
            vertices_3d[0],
            vertices_3d[1],
            vertices_3d[2],
        ))
        ...
        let indices = if is_ccw {
            Indices::U32(alloc::vec![0, 1, 2])
        } else {
            Indices::U32(alloc::vec![2, 1, 0])
        };
...
impl MeshBuilder for Capsule2dMeshBuilder {
    fn build(&self) -> Mesh {
        ...
        let normals = alloc::vec![[0.0, 0.0, 1.0]; vertex_count as usize];
...
mod tests {
    ...
    fn test_regular_polygon() {
        let mut mesh = RegularPolygon::new(7.0, 4).mesh();
    ...
    fn test_convex_polygon() {
        let polygon = ConvexPolygon::new(alloc::vec![
            Vec2::new(-2.0, -1.0),
            Vec2::new(2.0, -1.0),
            Vec2::new(1.0, 3.0),
            Vec2::new(-1.0, 2.0),
