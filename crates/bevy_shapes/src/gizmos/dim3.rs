...
use bevy_gizmos::{circles::SphereBuilder, gizmos::GizmoBuffer, prelude::GizmoConfigGroup};
    ...
    Clear: 'static + Send + Sync,
{
    fn drop(&mut self) {
        ...
        // This prevents allocations (no need for alloc crate) and cloning.
        let half_length = self.half_length;
        let radius = self.radius;
        let resolution = self.resolution;
        let isometry = self.isometry;
        let mk_upper_lower = move || {
            [-1.0, 1.0]
                .map(|sign| Vec3::Y * sign * half_length)
                .map(|vec3| {
                    circle_coordinates_closed(radius, resolution)
                        .map(move |vec2| Vec3::new(vec2.x, 0.0, vec2.y) + vec3)
                        .map(move |vec3| isometry * vec3)
                })
        };

        let [upper_points, lower_points] = mk_upper_lower();
        upper_points.skip(1).for_each(|start| {
            self.gizmos
                .short_arc_3d_between(upper_center, start, upper_apex, self.color);
        });
        lower_points.skip(1).for_each(|start| {
            self.gizmos
        ...
        let [upper_points, lower_points] = mk_upper_lower();
        let connection_lines = upper_points.zip(lower_points).skip(1);
    ...
    Clear: 'static + Send + Sync,
{
    fn drop(&mut self) {
        ...
        let circle_coords = || {
            circle_coordinates_closed(self.radius, self.height_resolution)
    ...
    Clear: 'static + Send + Sync,
{
    fn drop(&mut self) {
        ...
        let half_height = self.height * 0.5;
        let mk_upper_lower = || {
            [(-1.0, self.radius_bottom), (1.0, self.radius_top)].map(|(sign, radius)| {
                let translation = Vec3::Y * sign * half_height;
                circle_coordinates_closed(radius, self.resolution)
                    .map(move |vec2| Vec3::new(vec2.x, 0.0, vec2.y) + translation)
                    .map(|vec3| self.isometry * vec3)
            })
        };
        let [upper_points1, lower_points1] = mk_upper_lower();
        let [upper_points2, lower_points2] = mk_upper_lower();

        let upper_lines = upper_points1.zip(upper_points2.skip(1));
        let lower_lines = lower_points1.zip(lower_points2.skip(1));
    ...
    Clear: 'static + Send + Sync,
{
    fn drop(&mut self) {
        ...
        let [inner1, outer1, top1, bottom1] = mk_all_sides();
        let [inner2, outer2, top2, bottom2] = mk_all_sides();
        inner1
            .zip(inner2.skip(1))
            .chain(outer1.zip(outer2.skip(1)))
            .chain(top1.zip(top2.skip(1)))
            .chain(bottom1.zip(bottom2.skip(1)))
            .for_each(|(start, end)| {
                self.gizmos.line(start, end, self.color);
            });

        let [inner, outer, top, bottom] = mk_all_sides();
        inner
            .into_iter()
            .zip(top)
            .zip(outer)
