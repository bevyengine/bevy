...
impl<I: Index> Default for NamedHierarchy<I> {
    fn default() -> Self {
        Self {
                        ...
                        children
                            ...
                            .find(|entity| {
                                if let Some(entity_ref) = world.get_entity(**entity) {
                                    let current_parent = entity_ref.get::<Parent>();
                                    let name = entity_ref.get::<Name>();

                                    // ! FIXME: Parent changes before the children update it self,
                                    // ! to account for that we also must double check entity parent component it self
                                    if current_parent != Some(&Parent(parent_entity))
...
mod tests {
    ...
    use super::*;
    use crate::{
        hierarchy::{parent_update_system, BuildChildren},
        transform_propagate_system::transform_propagate_system,
    };

    #[test]
    fn find_entities_in_world() {
        ...
        let root = commands.spawn().insert(Name::new("root")).id();
        entities.push(root);

        commands.entity(root).with_children(|parent| {
            entities.push(parent.spawn().insert(Name::new("entity_1")).id());
            entities.push(parent.spawn().insert(Name::new("entity_2")).id());
        });

        commands.entity(entities[1]).with_children(|parent| {
            entities.push(parent.spawn().insert(Name::new("entity_3")).id());
        });

        command_queue.apply(&mut world);
        schedule.run(&mut world);

        let hierarchy = NamedHierarchy::from_ordered_entities(vec![
            (u16::MAX_VALUE, Name::new("root")),
            (0, Name::new("entity_1")),
            (0, Name::new("entity_2")),
            (1, Name::new("entity_3")),
        ]);
        ...
        let mut entities_table_cache = vec![];
        entities_table_cache.resize(hierarchy.len(), None);

        // ? NOTE: The root entity must be know prior the search
        entities_table_cache[0] = Some(root);

        for (i, entity) in entities.iter().enumerate().skip(1) {
            let found = hierarchy.find_entity_in_world(i as u16, &mut entities_table_cache, &world);
            assert_eq!(Some(*entity), found);
        }
    }

    #[test]
    fn merge_different_hierarchies() {
        let mut hierarchy_a = NamedHierarchy::<u16>::new();
        ...
        let mut hierarchy_b = NamedHierarchy::<u16>::new();
