...
/// Collection of methods similar to the built-in parenting methods on [`EntityWorldMut`] and
/// [`EntityCommands`], but preserving each entity's [`GlobalTransform`].
pub trait BuildChildrenTransformExt {
    ...
    ///
    /// Insert the [`ChildOf`] component directly if you don't want to also update the
    /// [`Transform`].
    ///
    ...
    ///
    /// See [`EntityWorldMut::remove_parent`] or [`EntityCommands::remove_parent`] for a method that
    /// doesn't update the [`Transform`].
    ///
    /// Note that both the hierarchy and transform updates will only execute
    /// the next time commands are applied
    /// (during [`ApplyDeferred`](bevy_ecs::schedule::ApplyDeferred)).
