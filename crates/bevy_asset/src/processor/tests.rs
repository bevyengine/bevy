...
fn only_reprocesses_wrong_hash_on_startup() {
    ...
    // Only source_changed, dep_changed, and multi_changed assets were reprocessed - all others
    // still have the same hashes.
    assert_eq!(*counter.lock().unwrap_or_else(PoisonError::into_inner), 3);
...
impl Process for FakeGltfSplitProcessor {
    ...
    async fn process(
        ...
        let gltf = context
            .load_source_asset::<FakeGltfLoader>(AssetMeta::new(AssetAction::Load {
                loader: "bevy_asset::tests::FakeGltfLoader".to_string(),
                settings: (),
            }))
            .await?;
