...
use crate::{
    ...
    LoadDirectError, LoadedAsset, LoadedUntypedAsset, UntypedHandle, AssetDependentOnSelf,
};
...
impl NestedLoader<'_, '_, StaticTyped, Deferred> {
    ...
    pub fn load<'c, A: Asset>(self, path: impl Into<AssetPath<'c>>) -> Handle<A> {
        ...
        if !is_self_path {
            ...
            warn!("Asset from path `{}` loaded the same path as a dependent, ignoring.", self.load_context.asset_path());
        }
        handle
    }
}
...
impl<'builder, 'reader, T> NestedLoader<'_, '_, T, Immediate<'builder, 'reader>> {
    ...
    async fn load_internal(
        ...
        if self.load_context.asset_path() == path {
            return Err(AssetDependentOnSelf { asset_path: path.clone() }.into());
        }
        self.load_context
            .asset_server
            .write_infos()
