use crate::io::{
    ...
    memory::Dir,
    AssetSourceEvent, AssetWatcher, FileWatcherEventType, FileWatcherEventPath,
};
...
impl FilesystemEventHandler for EmbeddedEventHandler {
    ...
    fn get_path(&self, absolute_path: &Path) -> Option<FileWatcherEventPath> {
        ...
        let final_path = self.root_paths.read().get(asset_path.path.as_path())?.clone();
        if asset_path.file_type != FileWatcherEventType::Asset {
            warn!("Meta file asset hot-reloading is not supported yet: {final_path:?}");
        }
        Some(FileWatcherEventPath {
            path: final_path,
            file_type: FileWatcherEventType::Asset,
        })
    }
