...
/// bytes have been embedded into the Rust binary using the
...
impl EmbeddedWatcher {
    /// Creates a new `EmbeddedWatcher` that watches for changes to the embedded assets in the given
    /// `dir`.
    pub fn new(
        dir: Dir,
...
/// binary-embedded Rust source files. This will read the contents of changed files from the file
