...
///
/// This uses
/// [`notify_debouncer_full`] to retrieve "debounced" filesystem events. "Debouncing" defines a time
/// window to hold on to events and then removes duplicate events that fall into this window.
/// This introduces a small delay in processing events, but it helps reduce event duplicates. A
/// small delay is also necessary on some systems to avoid processing a change event before it has
...
/// We want to try _very hard_ not to duplicate this event management logic across filesystem-driven
/// [`AssetWatcher`] impls.
///
/// Each operating system / platform behaves a little differently and this is the result of a
/// delicate balancing act that we should only perform once.
