...
impl AssetServer {
    ...
    pub fn register_asset<A: Asset>(&self, assets: &Assets<A>) {
        ...
        fn sender<A: Asset>(world: &mut World, index: AssetIndex) {
            world
                ...
                .send(AssetEvent::LoadedWithDependencies { id: index.into() });
        }
    ...
    /// labeled assets.
    fn send_loaded_asset(&self, index: ErasedAssetIndex, mut loaded_asset: ErasedLoadedAsset) {
        for (_, labeled_asset) in loaded_asset.labeled_assets.drain() {
            self.send_loaded_asset(
                // Labeled assets are always Strong variant, so this is safe.
                (&labeled_asset.handle).try_into().unwrap(),
                labeled_asset.asset,
            );
        }
    ...
    pub fn get_load_states(
        ...
        self.data.infos.read().get(index).map(|i| {
            (
                i.load_state.clone(),
    ...
    pub fn get_id_handle_untyped(&self, id: UntypedAssetId) -> Option<UntypedHandle> {
        ...
        self.data.infos.read().get_index_handle(index)
    }
    ...
    pub fn is_managed(&self, id: impl Into<UntypedAssetId>) -> bool {
        ...
        self.data.infos.read().contains_key(index)
    }
    ...
    pub fn get_path_ids<'a>(&self, path: impl Into<AssetPath<'a>>) -> Vec<UntypedAssetId> {
        let infos = self.data.infos.read();
        let path = path.into();
        infos.get_path_indices(&path).map(Into::into).collect()
    }
