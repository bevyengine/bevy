...
fn run_debug_asset_app(main_thread: Res<MainThreadExecutor>) {
    main_thread.run(|non_send_resources| {
        non_send_resources
            .resource_mut::<DebugAssetApp>()
            .0
            .update();
    });
}
...
pub(crate) fn sync_debug_assets<T: Asset + Clone>(
    ...
    main_thread.run(|non_send_resources| {
        let mut debug_asset_app = non_send_resources.resource_mut::<DebugAssetApp>();
