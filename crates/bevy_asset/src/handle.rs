...
///
/// When this is dropped, the [`Asset`] will be freed. It also stores
...
///
/// If a [`Handle`] is [`Handle::Strong`], the
/// [`Asset`] will be kept alive until the [`Handle`] is dropped. If a [`Handle`] is
/// [`Handle::Weak`], it does not necessarily reference a live [`Asset`], nor will it keep assets
/// alive.
///
...
///
/// [`Handle::Strong`] also provides access to useful [`Asset`] metadata, such as the [`AssetPath`]
/// (if it exists).
...
/// An untyped variant of [`Handle`] which stores the type information at runtime via [`TypeId`].
///
