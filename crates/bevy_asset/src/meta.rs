...
///
/// `L` is the [`AssetLoader`] (if one is configured) for the [`AssetAction`]. This can be `()` if
/// it is not required. `P` is the [`Process`] processor, if one is configured for the
/// [`AssetAction`]. This can be `()` if it is not required.
#[derive(Serialize, Deserialize)]
pub struct AssetMeta<L: AssetLoader, P: Process> {
    /// The version of the meta format being used. This will change whenever a breaking change is
    /// made to the meta format.
    pub meta_format_version: String,
    /// Information produced by the [`AssetProcessor`] _after_ processing this asset.
    /// This will only exist alongside processed versions of assets. You should not manually set it
    /// in your asset source files.
    ///
...
/// Info produced by the [`AssetProcessor`] for a given processed asset. This is used to determine
/// if an asset source file (or its dependencies) has changed.
///
/// [`AssetProcessor`]: crate::processor::AssetProcessor
...
pub struct ProcessedInfo {
    ...
    pub hash: AssetHash,
    /// A hash of the asset bytes, the asset .meta data, and the `full_hash` of every
    /// `process_dependency`
...
/// Information about a dependency used to process an asset. This is used to determine whether an
/// asset's "process dependency" has changed.
...
/// This is a minimal counterpart to [`AssetMeta`] that exists to speed up (or enable) serialization
/// in cases where the whole [`AssetMeta`] isn't necessary.
// PERF:
// Currently, this is used when retrieving asset loader and processor information (when the actual
// type is not known yet). This could probably be replaced (and made more efficient) by a custom
// deserializer that reads the loader/processor information _first_, then deserializes the contents
...
/// This is a minimal counterpart to [`AssetAction`] that exists to speed up (or enable)
/// serialization in cases where the whole [`AssetAction`] isn't necessary.
#[derive(Serialize, Deserialize)]
...
/// This is a minimal counterpart to [`ProcessedInfo`] that exists to speed up serialization in
/// cases where the whole [`ProcessedInfo`] isn't necessary.
#[derive(Serialize, Deserialize)]
pub struct ProcessedInfoMinimal {
    pub processed_info: Option<ProcessedInfo>,
}

/// A dynamic type-erased counterpart to [`AssetMeta`] that enables passing around and interacting
/// with [`AssetMeta`] without knowing its type.
...
/// The () processor should never be called. This implementation exists to make the meta format
/// nicer to work with.
impl Process for () {
    type Settings = ();
    type OutputLoader = ();
...
/// The () loader should never be called. This implementation exists to make the meta format nicer
/// to work with.
...
/// NOTE: changing the hashing logic here is a _breaking change_ that requires a
