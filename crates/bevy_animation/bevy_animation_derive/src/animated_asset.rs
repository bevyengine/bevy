use crate::{
    animated_properties::derive_animated_properties_for_asset,
...
pub fn derive_animated_asset(input: TokenStream) -> TokenStream {
    let animated_properties: TokenStream2 =
        derive_animated_properties_for_asset(input.clone()).into();
    ...
    let properties = fields
        .iter()
        .map(|field| format!("Handle<{}>.{}", struct_name, field.ident.as_ref().unwrap()));
    let field_types = fields.iter().map(|field| &field.ty);
    let fields = fields.iter().map(|field| field.ident.as_ref().unwrap());
    let handle_name = format!("Handle<{}>", struct_name);
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #bevy_animation::AnimatedAsset for #struct_name #ty_generics {
            fn animator_update_system(
                ...
                mut assets: #bevy_ecs::ResMut<#bevy_asset::Assets<Self>>,
                component_query: #bevy_ecs::Query<&mut Handle<Self>>,
            ) {
                    ...
                    for (_, layer, clip_handle, entities_map) in animator.animate() {
                        ...
                        if let Some(clip) = clips.get(clip_handle) {
                            ...
                            if let Some(curves) = clip
                                .get(#handle_name)
                                .map(|curve_untyped| curve_untyped.downcast_ref::<Handle<Self>>())
                                .flatten()
                            {
                                    ...
                                    for (entity_index, (curve_index, curve)) in curves.iter() {
                                        ...
                                        if let Some(ref component) = components[entity_index as usize] {
                                            if let Some(asset) = assets.get_mut(&**component) {
                                                ...
                                                keyframes[*curve_index] = k;
                                                asset.#fields.blend(&mut blend_group, v, w);
                                            }
                                        }
                                    }
                                }
