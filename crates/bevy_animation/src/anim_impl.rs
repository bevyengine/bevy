#[allow(unused_macros)]
macro_rules! anim_impl {
    (
        #[doc = $space: literal]

        type Rotation = $Rotation: ty;
        type Translation = $Translation: ty;
        type Scale = $Scale: ty;

        struct $AnimationClip: ident;
        struct $AnimationPlayer: ident;
        struct $AnimationPlugin: ident;
        struct $Keyframes: ident;
        struct $VariableCurve: ident;
    ) => {
        use std::ops::Deref;
        ...
        #[doc = "List of"]
        #[doc = $space]
        #[doc = "keyframes for one of the attributes of a [`Transform`]."]
        #[derive(Clone, Debug)]
        pub enum $Keyframes {
        ...
        #[doc = "Describes how an attribute of a [`Transform`] should be animated in"]
        #[doc = $space]
        #[doc = "."]
        #[doc = "\n"]
        #[doc = "`keyframe_timestamps` and `keyframes` should have the same length."]
        #[derive(Clone, Debug)]
        ...
        #[doc = "A list of [`"]
        #[doc = stringify!($VariableCurve)]
        #[doc = "`], and the [`EntityPath`] to which they apply."]
        ...
        #[doc = "System that will play all"]
        #[doc = $space]
        #[doc = "animations, using any entity with a [`$AnimationPlayer`]"]
        #[doc = "and a [`Handle<"]
        #[doc = stringify!($AnimationClip)]
        #[doc = ">`] as an animation root"]
                    ...
                    'entity: for (path, curves) in &animation_clip.curves {
                        ...
                        if let Ok(mut transform) = transforms.get_mut(current_entity) {
                            for curve in curves {
                                ...
                                if curve.keyframe_timestamps.len() == 1 {
                                    match &curve.keyframes {
                                        $Keyframes::Rotation(keyframes) => {
                                            transform.rotation = rotation_to_quat(keyframes[0]);
                                        }
                                        $Keyframes::Translation(keyframes) => {
                                            transform.translation =
                                                translation_to_vec3(keyframes[0]);
                                        }
                                        $Keyframes::Scale(keyframes) => {
                                            transform.scale = scale_to_vec3(keyframes[0]);
                                        }
                                    }
                                    continue;
                                }
                                ...
                                match &curve.keyframes {
                                    $Keyframes::Rotation(keyframes) => {
                                        let rot_start = rotation_to_quatlike(keyframes[step_start]);
                                        let mut rot_end =
                                            rotation_to_quatlike(keyframes[step_start + 1]);
                                        // Choose the smallest angle for the rotation
                                        if rot_end.dot(rot_start) < 0.0 {
                                            rot_end = -rot_end;
                                        }
                                        // Rotations are using a spherical linear interpolation
                                        transform.rotation = quatlike_to_quat(slerp_quatlike(
                                            rot_start.normalize(),
                                            rot_end.normalize(),
                                            lerp,
                                        ));
                                    ...
                                    $Keyframes::Translation(keyframes) => {
                                        ...
                                        transform.translation = translation_to_vec3(result);
                                    }
                                    $Keyframes::Scale(keyframes) => {
                                        let scale_start = keyframes[step_start];
                                        let scale_end = keyframes[step_start + 1];
                                        let result = scale_start.lerp(scale_end, lerp);
                                        transform.scale = scale_to_vec3(result);
                                    }
                                }
                            }
                        }
        ...
        #[doc = "Adds"]
        #[doc = $space]
        #[doc = "animation support to an app"]
        #[derive(Default)]
