...
pub fn extract_views(
    ...
    views: Extract<Query<(Entity, RenderEntity, &View, Option<&ViewTarget>)>>,
) {
    for (main_entity, render_entity, view, view_target) in &views {
        let extracted_view = view_target
            .filter(|_| view.is_enabled())
            .map(|view_target| ExtractedView {
        ...
        if let Some(extracted_view) = extracted_view {
