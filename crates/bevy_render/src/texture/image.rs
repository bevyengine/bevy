...
impl Image {
    ...
    pub fn is_compressed(&self) -> bool {
        ...
        format_description
            .required_features
            .contains(WgpuFeatures::TEXTURE_COMPRESSION_ASTC_LDR)
            || format_description
                .required_features
                .contains(WgpuFeatures::TEXTURE_COMPRESSION_BC)
            || format_description
                .required_features
                .contains(WgpuFeatures::TEXTURE_COMPRESSION_ETC2)
    }
}
...
impl CompressedImageFormats {
    pub fn from_features(features: WgpuFeatures) -> Self {
        let mut supported_compressed_formats = Self::default();
        if features.contains(WgpuFeatures::TEXTURE_COMPRESSION_ASTC_LDR) {
            supported_compressed_formats |= Self::ASTC_LDR;
        }
        if features.contains(WgpuFeatures::TEXTURE_COMPRESSION_BC) {
            supported_compressed_formats |= Self::BC;
        }
        if features.contains(WgpuFeatures::TEXTURE_COMPRESSION_ETC2) {
