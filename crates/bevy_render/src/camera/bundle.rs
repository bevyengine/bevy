...
impl OrthographicCameraBundle {
    ...
    /// Y=0.
    ///
    /// The scaling mode is the default for [`OrthographicProjection`], resulting in
    /// a resolution where 1 unit in X and Y in camera space corresponds to 1 pixel on
    /// the screen. That is, for a screen of 1920 pixels in width, the X coordinates
    /// visible on screen go from X=-960 to X=+960 in world space, left to right.
    ///
