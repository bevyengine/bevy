...
Dimensions:
- Conflict: InheritedRenderGroups and PropagateRenderGroups on the same entity
...
propagator is encountered.
    ...
    - PROBLEM: If a new entity is inserted between a propagator and another entity, then its parent will run in this
    step, and its non-propagator children will run in "If a non-propagator entity is parented to a non-propagator".
    This step will insert an InheritedRenderGroups component to the new entity, but it *won't* be added to the
    pre-existing entity which already records the propagator. When the pre-eisting entity gets updated because of
    being parented to a non-propagator, the parent's InheritedRenderGroups component won't be available since its
...
children (stopping at propagators). Skip already-updated entities.
    - PROBLEM: What if multiple entities in a hierarchy lose PropagateRenderGroups components?
        ...
        This will cause some redundant work, but gives the correct result.
            ...
            the desired policy (regardless of it the entity has InheritedRenderGroups), in case previous updates
            of this type need to be overwritten.
...
fn apply_full_propagation(
    ...
    // Leave if child doesn't exist or has PropagateRenderGroups.
    let Ok(maybe_render_groups, mut maybe_inherited_groups) = maybe_inherited.get_mut(child) else {
        return;
    }
    ...
    let Ok(children) = children_query.get(child) else {
        return;
    };
    for child in children.iter() {
        apply_full_propagation(
