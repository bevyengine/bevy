...
use crate::{
    ...
    render_resource::{
        BindGroup, DynamicArrayIndex, DynamicArrayUniformBuffer, ShaderType, Texture, TextureView,
        TextureViewId,
    },
...
impl ViewTarget {
    ...
    pub fn get_unsampled_color_attachment_for_layer(
        ...
        if self.main_texture.load(Ordering::SeqCst) == 0 {
            self.main_textures
                .a
                .get_unsampled_attachment_for_layer(layer)
        } else {
            self.main_textures
                .b
                .get_unsampled_attachment_for_layer(layer)
...
pub fn prepare_view_uniforms(
    ...
    ) in &views
    {
        ...
        let lod_view_world_position = match (&extracted_camera, &shadow_lod_origin) {
            ...
            (None, Some(shadow_lod_origin)) => {
                ...
                {
                    Some((_, _, camera_view, _, _, _, _, _)) => {
    ...
    for (entity, array_index) in per_entity {
        ...
        commands.entity(entity).insert(ViewUniformOffset { offset });
    }
}
