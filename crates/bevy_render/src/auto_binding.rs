...
// object-safe wrapper for implementing DynAutoBinding, stored as a dyn vec in
// AutoBindings<G> and used to construct the layout
...
// globally unique mapping of shader module+var name to binding slot
#[derive(Resource, Default)]
...
// store for layout for all AutoBindings in an auto bind group
#[derive(Resource)]
...
// stores the OwnedBindingResources generated by queue_binding
#[derive(Component)]
pub struct AutoBindingData<G: AutoBindGroup> {
    // key is the binding slot, value is the binding resource
    binding_sources: BTreeMap<u32, Option<OwnedBindingResource>>,
    _p: PhantomData<fn(&G)>,
}

// system to initialize the AutoBindingData in Prepare stage so that
// queue systems can mutably insert themselves in the Queue stage, and data is available for
// building the final bind group, also in the Queue stage.
...
// api for adding automatic bindings
...
// system to fetch the OwnedBindingResource for a binding in the bind group
...
// system to generate the final bind group
...
// RenderCommand to set an auto bind group for the current view entity
