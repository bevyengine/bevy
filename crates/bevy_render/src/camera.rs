...
use bevy_camera::{
    ...
    Projection, RenderTargetInfo, SubViewSourceProjection, SubViewsUsingThisProjection, Viewport,
};
...
pub fn camera_system(
    ...
    for (entity, mut camera, mut camera_projection, sub_views, has_source) in &mut cameras {
    ...
    for entity in has_sub_view_source {
        let Ok((entity, mut camera, mut camera_projection, sub_views, Some(sub_view_source))) =
            cameras.get(entity)
        else {
            continue;
        };

        let Ok((_, _, source_projection, Some(_), _)) = cameras.get(sub_view_source.0) else {
            continue;
        };
    ...
    for (entity, thing) in calculated_things {
        let (_, mut camera, _, _, _) = cameras.get_mut(entity).unwrap();
