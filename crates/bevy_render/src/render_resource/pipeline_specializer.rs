use crate::{
    ...
    render_resource::{
        ComputePipelineDescriptor, ComputePipelineId, PipelineCache, RenderPipelineDescriptor,
        RenderPipelineId, VertexBufferLayout,
    },
};
...
pub struct SpecializedRenderPipelines<S: SpecializedRenderPipeline> {
    cache: HashMap<S::Key, RenderPipelineId>,
}
...
pub struct SpecializedComputePipelines<S: SpecializedComputePipeline> {
    cache: HashMap<S::Key, ComputePipelineId>,
}
...
pub struct SpecializedMeshPipelines<S: SpecializedMeshPipeline> {
    mesh_layout_cache: PreHashMap<InnerMeshVertexBufferLayout, HashMap<S::Key, RenderPipelineId>>,
    vertex_layout_cache: HashMap<VertexBufferLayout, HashMap<S::Key, RenderPipelineId>>,
}
...
impl<S: SpecializedMeshPipeline> SpecializedMeshPipelines<S> {
    ...
    pub fn specialize(
        ...
        key: S::Key,
        layout: &MeshVertexBufferLayout,
    ) -> Result<RenderPipelineId, SpecializedMeshPipelineError> {
        let map = self
            .mesh_layout_cache
                ...
                Ok(*entry.insert(match layout_map.entry(key) {
                    Entry::Occupied(entry) => {
                        if cfg!(debug_assertions) {
                            let stored_descriptor = cache.render_pipeline_descriptor(*entry.get());
