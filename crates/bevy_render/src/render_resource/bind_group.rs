...
/// [`Texture`](crate::render_resource::Texture) for an [`Image`](crate::texture::Image) hasn't
/// loaded yet), just return [`AsBindGroupError::RetryNextUpdate`], which signals that the caller
...
///       written to a [`Buffer`], and bound as a uniform. [`ShaderType`] is implemented for most
///       math types already, such as [`f32`], [`Vec4`](bevy_math::Vec4), and
...
///       [`Texture`](crate::render_resource::Texture) GPU resource, which will be bound as a
///       texture in shaders. The field will be assumed to implement
...
///       [`Option<Handle<Image>>`] is [`None`], the [`crate::texture::FallbackImage`] resource will
///       be used instead. This attribute can be used in conjunction with a `sampler` binding
///       attribute (with a different binding index) if a binding of the sampler for the
///       [`Image`](crate::texture::Image) is also required.
///
...
///     * This field's [`Handle<Image>`](bevy_asset::Handle) will be used to look up the matching
///       [`Texture`](crate::render_resource::Texture) GPU resource, which will be bound as a
///       storage texture in shaders. The field will be assumed to implement
///       [`Into<Option<Handle<Image>>>`]. In practice, most fields should be a
///       [`Handle<Image>`](bevy_asset::Handle) or [`Option<Handle<Image>>`]. If the value of an
///       [`Option<Handle<Image>>`] is [`None`], the [`crate::texture::FallbackImage`] resource will
///       be used instead.
...
///       [`Handle<Image>`](bevy_asset::Handle) or [`Option<Handle<Image>>`]. If the value of an
///       [`Option<Handle<Image>>`] is [`None`], the [`crate::texture::FallbackImage`] resource will
///       be used instead. This attribute can be used in conjunction with a `texture` binding
///       attribute (with a different binding index) if a binding of the texture for the
///       [`Image`](crate::texture::Image) is also required.
///
/// | Arguments              | Values                                                                  | Default                |
/// |------------------------|-------------------------------------------------------------------------|------------------------|
...
///       [`Buffer`] GPU resource, which will be bound as a storage buffer in shaders. If the
///       `storage` attribute is used, the field is expected a raw buffer, and the buffer will be
///       bound as a storage buffer in shaders.
...
///     * This also creates a [`Buffer`] using [`ShaderType`] and binds it as a uniform, much like
///       the field-level `uniform` attribute. The difference is that the entire [`AsBindGroup`]
///       value is converted to `ConvertedShaderType`, which must implement [`ShaderType`], instead
///       of a specific field implementing [`ShaderType`]. This is useful if more complicated
///       conversion logic is required. The conversion is done using the
...
///       access to resources like [`RenderAssets<GpuImage>`] is required.
/// * `bind_group_data(DataType)`
///     * The [`AsBindGroup`] type will be converted to some `DataType` using [`Into<DataType>`] and
///       stored as [`AsBindGroup::Data`] as part of the [`AsBindGroup::as_bind_group`] call. This
///       is useful if data needs to be stored alongside the generated bind group, such as a unique
///       identifier for a material's bind group. The most common use case for this attribute is
