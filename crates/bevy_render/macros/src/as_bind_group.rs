...
pub fn derive_as_bind_group(ast: syn::DeriveInput) -> Result<TokenStream> {
        ...
        if let Some(attr_ident) = attr.path().get_ident() {
            if attr_ident == BIND_GROUP_DATA_ATTRIBUTE_NAME {
                ...
                binding_impls.push(quote! {{
                    use #render_path::gpu_resource::AsBindGroupShaderType;
                    let mut buffer = #render_path::gpu_resource::encase::UniformBuffer::new(Vec::new());
                    let converted: #converted_shader_type = self.as_bind_group_shader_type(images);
                    buffer.write(&converted).unwrap();
                    (
                        #binding_index,
                        #render_path::gpu_resource::OwnedBindingResource::Buffer(gpu_device.create_buffer_with_data(
                            &#render_path::gpu_resource::BufferInitDescriptor {
                                label: None,
                                usage: #render_path::gpu_resource::BufferUsages::COPY_DST | #render_path::gpu_resource::BufferUsages::UNIFORM,
                ...
                binding_layouts.push(quote!{
                    #render_path::gpu_resource::BindGroupLayoutEntry {
                        binding: #binding_index,
                        visibility: #render_path::gpu_resource::ShaderStages::all(),
                        ty: #render_path::gpu_resource::BindingType::Buffer {
                            ty: #render_path::gpu_resource::BufferBindingType::Uniform,
                            has_dynamic_offset: false,
                            min_binding_size: Some(<#converted_shader_type as #render_path::gpu_resource::ShaderType>::min_size()),
                        },
                        count: None,
                    }
                });
        ...
        for attr in &field.attrs {
            ...
            match binding_type {
                ...
                BindingType::Storage => {
                    ...
                    let visibility =
                        visibility.hygienic_quote(&quote! { #render_path::gpu_resource });
                    ...
                    let min_binding_size = if buffer {
                        ...
                        quote! {Some(<#field_ty as #render_path::gpu_resource::ShaderType>::min_size())}
                    };
                    ...
                    if buffer {
                        binding_impls.push(quote! {
                            (
                                #binding_index,
                                #render_path::gpu_resource::OwnedBindingResource::Buffer({
                        ...
                        binding_impls.push(quote! {{
                            use #render_path::gpu_resource::AsBindGroupShaderType;
                            let mut buffer = #render_path::gpu_resource::encase::StorageBuffer::new(Vec::new());
                            ...
                            (
                                ...
                                #render_path::gpu_resource::OwnedBindingResource::Buffer(gpu_device.create_buffer_with_data(
                                    &#render_path::gpu_resource::BufferInitDescriptor {
                                        label: None,
                                        usage: #render_path::gpu_resource::BufferUsages::COPY_DST | #render_path::gpu_resource::BufferUsages::STORAGE,
                                        contents: buffer.as_ref(),
                                    },
                                ))
                            )
                    ...
                    binding_layouts.push(quote! {
                        #render_path::gpu_resource::BindGroupLayoutEntry {
                            ...
                            ty: #render_path::gpu_resource::BindingType::Buffer {
                                ty: #render_path::gpu_resource::BufferBindingType::Storage { read_only: #read_only },
                                has_dynamic_offset: false,
                ...
                BindingType::Texture => {
                    ...
                    binding_impls.push(quote! {
                        (
                            #binding_index,
                            #render_path::gpu_resource::OwnedBindingResource::TextureView({
                                let handle: Option<&#asset_path::Handle<#render_path::texture::Image>> = (&self.#field_name).into();
                                if let Some(handle) = handle {
                                    images.get(handle).ok_or_else(|| #render_path::gpu_resource::AsBindGroupError::RetryNextUpdate)?.texture_view.clone()
                                } else {
                    ...
                    binding_layouts.push(quote! {
                        #render_path::gpu_resource::BindGroupLayoutEntry {
                            binding: #binding_index,
                            visibility: #visibility,
                            ty: #render_path::gpu_resource::BindingType::Texture {
                                multisampled: #multisampled,
                                sample_type: #render_path::gpu_resource::#sample_type,
                                view_dimension: #render_path::gpu_resource::#dimension,
                            },
                            count: None,
                        }
                    });
                ...
                BindingType::Sampler => {
                    ...
                    binding_impls.push(quote! {
                        (
                            #binding_index,
                            #render_path::gpu_resource::OwnedBindingResource::Sampler({
                                let handle: Option<&#asset_path::Handle<#render_path::texture::Image>> = (&self.#field_name).into();
                                if let Some(handle) = handle {
                                    images.get(handle).ok_or_else(|| #render_path::gpu_resource::AsBindGroupError::RetryNextUpdate)?.sampler.clone()
                                } else {
                    ...
                    binding_layouts.push(quote!{
                        #render_path::gpu_resource::BindGroupLayoutEntry {
                            binding: #binding_index,
                            visibility: #visibility,
                            ty: #render_path::gpu_resource::BindingType::Sampler(#render_path::gpu_resource::#sampler_binding_type),
                            count: None,
                        }
                    });
                }
        ...
        if let BindingState::OccupiedMergeableUniform { uniform_fields } = binding_state {
                ...
                binding_layouts.push(quote!{
                    #render_path::gpu_resource::BindGroupLayoutEntry {
                        ...
                        ty: #render_path::gpu_resource::BindingType::Buffer {
                            ty: #render_path::gpu_resource::BufferBindingType::Uniform,
                            has_dynamic_offset: false,
                            min_binding_size: Some(<#field_ty as #render_path::gpu_resource::ShaderType>::min_size()),
                        },
                        count: None,
                    }
                });
            // multi-field uniform bindings for a given index require an intermediate struct to derive ShaderType
                ...
                field_struct_impls.push(quote! {
                    #[derive(#render_path::gpu_resource::ShaderType)]
                ...
                binding_layouts.push(quote!{
                    #render_path::gpu_resource::BindGroupLayoutEntry {
                        ...
                        ty: #render_path::gpu_resource::BindingType::Buffer {
                            ty: #render_path::gpu_resource::BufferBindingType::Uniform,
                            has_dynamic_offset: false,
                            min_binding_size: Some(<#uniform_struct_name as #render_path::gpu_resource::ShaderType>::min_size()),
                        },
                        count: None,
                    }
                });
    ...
    Ok(TokenStream::from(quote! {
        ...
        impl #impl_generics #render_path::gpu_resource::AsBindGroup for #struct_name #ty_generics #where_clause {
            ...
            fn unprepared_bind_group(
                &self,
                layout: &#render_path::gpu_resource::BindGroupLayout,
            ...
            ) -> Result<#render_path::gpu_resource::UnpreparedBindGroup<Self::Data>, #render_path::gpu_resource::AsBindGroupError> {
                let bindings = vec![#(#binding_impls,)*];

                Ok(#render_path::gpu_resource::UnpreparedBindGroup {
                    bindings,
                    data: #get_prepared_data,
                })
            }

            fn bind_group_layout_entries(gpu_device: &#render_path::renderer::GpuDevice) -> Vec<#render_path::gpu_resource::BindGroupLayoutEntry> {
                vec![#(#binding_layouts,)*]
            }
        }
    }))
...
fn get_fallback_image(
    ...
    quote! {
        match #render_path::gpu_resource::#dimension {
            #render_path::gpu_resource::TextureViewDimension::D1 => &fallback_image.d1,
            #render_path::gpu_resource::TextureViewDimension::D2 => &fallback_image.d2,
            #render_path::gpu_resource::TextureViewDimension::D2Array => &fallback_image.d2_array,
            #render_path::gpu_resource::TextureViewDimension::Cube => &fallback_image.cube,
            #render_path::gpu_resource::TextureViewDimension::CubeArray => &fallback_image.cube_array,
            #render_path::gpu_resource::TextureViewDimension::D3 => &fallback_image.d3,
        }
    }
}
