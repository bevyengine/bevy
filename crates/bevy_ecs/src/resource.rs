...
mod tests {
    ...
    fn is_resource_presence() {
        ...
        assert_eq!(
            ...
            "The resource should always be inserted on the component entity"
        );
    ...
    fn get_multiple_dynamic_resources() {
        ...
        fn resource_system(query: Query<FilteredEntityRef>, components: &Components) {
            let component_id_a = components.get_id(TypeId::of::<ResA>()).unwrap();
            let component_id_b = components.get_id(TypeId::of::<ResB>()).unwrap();

            let entity_a = component_id_a.entity();
            let entity_b = component_id_b.entity();
