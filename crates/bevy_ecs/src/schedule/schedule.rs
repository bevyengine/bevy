...
impl ScheduleGraph {
    ...
    fn add_system_inner(&mut self, config: SystemConfig) -> Result<NodeId, ScheduleBuildError> {
        ...
        self.uninit.push(PendingInit::System(id.index()));
    ...
    fn configure_set_inner(&mut self, set: SystemSetConfig) -> Result<NodeId, ScheduleBuildError> {
        ...
        self.uninit.push(PendingInit::SetCondition {
            set_index: id.index(),
            condition_index: system_set_conditions.len(),
        });
    ...
    pub fn initialize(&mut self, world: &mut World) {
        for uninit in self.uninit.drain(..) {
            match uninit {
                PendingInit::System(index) => {
                    let id = NodeId::System(index);
