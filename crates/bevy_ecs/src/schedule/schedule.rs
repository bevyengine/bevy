...
pub struct ScheduleGraph {
    ...
    pub system_conditions: SecondaryMap<SystemKey, Vec<BoxedCondition>>,
    ...
    system_set_conditions: SecondaryMap<SystemSetKey, Vec<BoxedCondition>>,
...
impl ScheduleGraph {
    ...
    pub fn get_system_at(&self, key: SystemKey) -> Option<&ScheduleSystem> {
        self.systems.get(key).and_then(|system| system.get())
    }
    ...
    pub fn get_set_conditions_at(&self, key: SystemSetKey) -> Option<&[BoxedCondition]> {
    ...
    /// Panics if it doesn't exist.
    #[track_caller]
    pub fn set_conditions_at(&self, key: SystemSetKey) -> &[BoxedCondition] {
    ...
    pub fn systems(&self) -> impl Iterator<Item = (SystemKey, &ScheduleSystem, &[BoxedCondition])> {
    ...
    pub fn system_sets(
        &self,
    ) -> impl Iterator<Item = (SystemSetKey, &dyn SystemSet, &[BoxedCondition])> {
    ...
    fn add_system_inner(
        ...
        self.system_conditions.insert(key, config.conditions);
        ...
        for id in self.uninit.drain(..) {
            match id {
                UninitializedId::System(key) => {
                    self.systems[key].get_mut().unwrap().initialize(world);
...
impl ScheduleGraph {
    ...
    fn get_node_name_inner(&self, id: &NodeId, report_sets: bool) -> String {
        let name = match *id {
            NodeId::System(key) => {
                let name = self.systems[key].get().unwrap().name().to_string();
                if report_sets {
