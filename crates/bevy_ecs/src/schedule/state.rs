...
mod test {
    ...
    fn state_test() {
        ...
        stage.run(&mut world);
        let mut collected = world.get_resource_mut::<Vec<&'static str>>();
        let mut count = 0;
        ...
        assert_eq!(
            world.get_resource::<State<MyState>>().current(),
            &MyState::Final
        );
    }
    ...
    fn issue_1753() {
        ...
        stage.run(&mut world);
        assert!(*world.get_resource::<bool>(), "after control");
        ...
        stage.run(&mut world);
        assert!(*world.get_resource::<bool>(), "after test");
    }
    ...
    fn restart_state_tests() {
        ...
        // A. Restart state
        let mut state = world.get_resource_mut::<State<LoadState>>();
        ...
        let mut collected = world.get_resource_mut::<Vec<LoadStatus>>();
        let mut count = 0;
        for (found, expected) in collected.drain(..).zip(EXPECTED) {
            assert_eq!(found, *expected);
            count += 1;
        ...
        assert_eq!(EXPECTED.len(), count);
        assert_eq!(
            world.get_resource::<State<LoadState>>().current(),
            &LoadState::Finish
        );
    }
}
