...
pub struct Labels(
    // This type-map stores instances of `IndexSet<T>`, for any fully-resolved type `T`.
    //
    // The `IndexSet` is what interns the values for each type -- it's a hash set
    // that preservers ordering as long as you don't remove items (which we don't).
    // This allows us to use O(~1) hashing and map each entry to a stable index.
    RwLock<TypeMap>,
);
