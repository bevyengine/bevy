...
/// A flat, type-erased data storage type similar to a [`BlobVec`](super::blob_vec::BlobVec), but
/// with the length and capacity cut out for performance reasons. This type is reliant on its owning
/// type to store the capacity and length information.
///
...
/// string, arrays, etc). This type only stores meta-data about the Blob that it stores,
...
impl BlobArray {
    ...
    /// [`BlobArray`] should be dropped. For all Rust-based types, this should match 1:1 with
    /// the implementation of [`Drop`] if present, and should be `None` if `T: !Drop`. For
    /// non-Rust based types, this should match any cleanup processes typically associated with
    /// the stored element.
    ///
    /// # Safety
    /// `drop` should be safe to call with an [`OwningPtr`] pointing to any item that's been placed
    /// into this [`BlobArray`]. If `drop` is `None`, the items will be leaked. This should
    /// generally be set as None based on [`needs_drop`].
    ///
    ...
    ///
    /// *`len` refers to the length of the array, the number of elements that have been initialized,
    /// and are safe to read. Just like [`Vec::len`], or
    /// [`BlobVec::len`](super::blob_vec::BlobVec::len).*
    ///
    /// # Safety
    /// - The element at index `index` is safe to access. (If the safety requirements of every
    ///   method that has been used on `Self` have been fulfilled, the caller just needs to ensure
    ///   that `index` < `len`)
    ///
    ...
    pub unsafe fn get_unchecked(&self, index: usize) -> Ptr<'_> {
        ...
        // SAFETY:
        // - The caller ensures that `index` fits in this array, so this operation will not overflow
    ...
    ///
    /// # Safety
    /// - The element with at index `index` is safe to access. (If the safety requirements of every
    ///   method that has been used on `Self` have been fulfilled, the caller just needs to ensure
    ///   that `index` < `len`)
    ///
    /// [`Vec::len`]: alloc::vec::Vec::len
    ...
    /// Get a slice of the first `slice_len` elements in [`BlobArray`] as if it were an array with
    /// elements of type `T` To get a slice to the entire array, the caller must plug `len` in
    /// `slice_len`.
    ///
    /// *`len` refers to the length of the array, the number of elements that have been initialized,
    /// and are safe to read. Just like [`Vec::len`], or
    ...
    ///
    /// # Safety
    /// - For every element with index `i`, if `i` < `len`: It must be safe to call
    ///   [`Self::get_unchecked_mut`] with `i`. (If the safety requirements of every method that has
    ///   been used on `Self` have been fulfilled, the caller just needs to ensure that `len` is
    ///   correct.)
    ///
    ...
    pub unsafe fn clear(&mut self, len: usize) {
        ...
        if let Some(drop) = self.drop {
            // We set `self.drop` to `None` before dropping elements for unwind safety. This ensures
            // we don't accidentally drop elements twice in the event of a drop impl
            // panicking.
            self.drop = None;
            let size = self.item_layout.size();
            for i in 0..len {
                ...
                // SAFETY: `item` was obtained from this `BlobArray`, so its underlying type must
                // match `drop`.
                unsafe { drop(item) };
            }
            self.drop = Some(drop);
    ...
    /// Allocate a block of memory for the array. This should be used to initialize the array, do
    /// not use this method if there are already elements stored in the array - use
    /// [`Self::realloc`] instead.
    pub(super) fn alloc(&mut self, capacity: NonZeroUsize) {
        ...
        if !self.is_zst() {
            ...
            // SAFETY: layout has non-zero size because capacity > 0, and the blob isn't ZST
            // (`self.is_zst` == false)
    ...
    /// For example, if the length (number of stored elements) reached the capacity (number of
    /// elements the current allocation can store), you might want to use this method to
    /// increase the allocation, so more data can be stored in the array.
    ///
    /// # Safety
    /// - `current_capacity` is indeed the current capacity of this array.
    /// - After calling this method, the caller must update their saved capacity to reflect the
    ///   change.
    pub(super) unsafe fn realloc(
        ...
        if !self.is_zst() {
            ...
            // - the layout used to previously allocate this array is equivalent to
            //   `array_layout(&self.item_layout, current_capacity.get())`
            // - `item_layout.size() > 0` (`self.is_zst`==false) and `new_capacity > 0`, so the
            ...
            let new_data = unsafe {
                alloc::alloc::realloc(
                    ...
                    // SAFETY: This is the Layout of the current array, it must be valid, if it
                    // hadn't have been, there would have been a panic on a previous allocation
    ...
    /// - The [`Layout`] of the value must match the layout of the blobs stored in this array, and
    ///   it must be safe to use the `drop` function of this [`BlobArray`] to drop `value`.
    ...
    /// - `value`'s [`Layout`] must match this [`BlobArray`]'s `item_layout`, and it must be safe to
    ///   use the `drop` function of this [`BlobArray`] to drop `value`.
    ...
    pub unsafe fn swap_remove_unchecked(
        ...
        // Now the element that used to be in index `index_to_remove` is now in index
        // `index_to_keep` (after swap) If we are storing ZSTs than the index doesn't
        // actually matter because the size is 0.
    ...
    /// The same as [`Self::swap_remove_and_drop_unchecked`] but the two elements must
    /// non-overlapping.
    ///
    /// # Safety
    /// - `index_to_keep` must be safe to access (within the bounds of the length of the array).
