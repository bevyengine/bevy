...
/// following an infinite traversal loop without an eventual exit will cause your application to
/// hang. Each implementer of `Traversal` for documenting possible looping behavior, and consumers
/// of those implementations are responsible for avoiding infinite loops in their code.
///
/// Traversals may be parameterized with additional data. For example, in observer event
/// propagation, the parameter `D` is the event type given in `Trigger<E>`. This allows traversal to
/// differ depending on event data.
///
