...
/// ```
///
/// Note that a system will panic if an `EventWriter` is used as a `SystemParam` without a
/// corresponding [`Events<E>`] resource in the [`World`]. For cases where the `Events` resource
/// may or may not exist, `Option<EventWriter<E>>` can be used instead.
///
/// # Limitations
///
...
/// It simply wraps an internal SystemParamState, similar to the FetchState generated by
/// `#[derive(SystemParam)]`.
#[doc(hidden)]
