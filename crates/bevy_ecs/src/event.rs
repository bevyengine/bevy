...
/// # Panics
///
/// Panics when used as a [`SystemParameter`](SystemParam) if a matching [`Events<E>`] does not exist.
///
/// Use `Option<EventReader<E>>` instead if the Events resource might not always exist.
#[derive(SystemParam)]
...
/// ```
///
/// Note that a system will panic if an `EventWriter` is used as a `SystemParam` without a corresponding
/// [`Events<E>`] resource in the [`World`].
/// For cases where the `Events` resource may or may not exist, `Option<EventWriter<E>>` can be used instead.
///
/// # Limitations
///
...
/// The [`SystemParamState`] of [`Option<EventReader<E>>`] and [`Option<EventWriter<E>>`].
/// It simply wraps an internal SystemParamState, similar to the `FetchState` generated by `#[derive(SystemParam)]`.
#[doc(hidden)]
pub struct OptionEventStateWrapper<T, E: Event> {
    state: T,
    marker: PhantomData<fn() -> E>,
}

// SAFETY: this impl simply defers to the init function of the nested state T
unsafe impl<T: SystemParamState, E: Event> SystemParamState for OptionEventStateWrapper<T, E> {
    fn init(world: &mut World, system_meta: &mut SystemMeta) -> Self {
        Self {
            state: T::init(world, system_meta),
            marker: PhantomData,
        }
    }
}
...
impl<'w, 's, E: Event> SystemParam for Option<EventReader<'w, 's, E>> {
    type Fetch = OptionEventStateWrapper<<EventReader<'w, 's, E> as SystemParam>::Fetch, E>;
}

impl<'w, 's, E: Event> SystemParamFetch<'w, 's>
    for OptionEventStateWrapper<<EventReader<'w, 's, E> as SystemParam>::Fetch, E>
{
    type Item = Option<EventReader<'w, 's, E>>;
    ...
    unsafe fn get_param(
        ...
        world.get_resource::<Events<E>>().map(|_event| {
            <EventReader<'w, 's, E> as SystemParam>::Fetch::get_param(
                &mut state.state,
                system_meta,
                world,
                change_tick,
...
impl<'w, 's, E: Event> SystemParam for Option<EventWriter<'w, 's, E>> {
    type Fetch = OptionEventStateWrapper<<EventWriter<'w, 's, E> as SystemParam>::Fetch, E>;
}

impl<'w, 's, E: Event> SystemParamFetch<'w, 's>
    for OptionEventStateWrapper<<EventWriter<'w, 's, E> as SystemParam>::Fetch, E>
{
    type Item = Option<EventWriter<'w, 's, E>>;
...
mod tests {
    ...
    #[test]
    fn test_optional_event_reader_present() {
        use bevy_ecs::prelude::*;
    ...
    #[test]
    fn test_optional_event_reader_not_present() {
        use bevy_ecs::prelude::*;
    ...
    #[test]
    fn test_optional_event_writer_present() {
        use bevy_ecs::prelude::*;
    ...
    #[test]
    fn test_optional_event_writer_not_present() {
        use bevy_ecs::prelude::*;
