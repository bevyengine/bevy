...
/// `#[derive(SystemParam)]`.
#[doc(hidden)]
pub struct OptionEventAccessState<T, E: Event> {
    state: T,
    marker: PhantomData<fn() -> E>,
}

// SAFETY: this impl simply defers to the init function of the nested state T
unsafe impl<T: SystemParamState, E: Event> SystemParamState for OptionEventAccessState<T, E> {
...
impl<'w, 's, E: Event> SystemParam for Option<EventReader<'w, 's, E>> {
    type Fetch = OptionEventAccessState<<EventReader<'w, 's, E> as SystemParam>::Fetch, E>;
}

impl<'w, 's, E: Event> SystemParamFetch<'w, 's>
    for OptionEventAccessState<<EventReader<'w, 's, E> as SystemParam>::Fetch, E>
{
...
impl<'w, 's, E: Event> SystemParam for Option<EventWriter<'w, 's, E>> {
    type Fetch = OptionEventAccessState<<EventWriter<'w, 's, E> as SystemParam>::Fetch, E>;
}

impl<'w, 's, E: Event> SystemParamFetch<'w, 's>
    for OptionEventAccessState<<EventWriter<'w, 's, E> as SystemParam>::Fetch, E>
{
