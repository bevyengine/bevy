...
/// structs that implement [`Fetch`] and are used as [`WorldQuery::Fetch`](WorldQueryGats::Fetch) and
...
/// [`Fetch::matches_component_set`], [`Fetch::archetype_fetch`], and
/// [`Fetch::table_fetch`].
pub unsafe trait Fetch<'world>: Sized {
    ...
    /// # Safety
    ///
    /// `state` must have been initialized (via [`Fetch::init_state`]) using the same `world` passed
    /// in to this function.
    unsafe fn init(
        world: &'world World,
        state: &Self::State,
...
unsafe impl<'w, T: Fetch<'w>> Fetch<'w> for OptionFetch<T> {
    ...
    unsafe fn init(
        ...
        Self {
            fetch: T::init(world, state, last_change_tick, change_tick),
            matches: false,
        }
    }
