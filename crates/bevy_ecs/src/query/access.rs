...
mod tests {
    ...
    fn invertible_union_with_different_lengths() {
        ...
        let mut self_set = set_from_u32s(vec![0]);
        let mut self_inverted = false;
        let other_set = set_from_u32s(vec![0, 1]);
        ...
        // [0] | [2, ...] = [0, 2, ...]
        assert_eq!((self_set, self_inverted), (set_from_u32s(vec![1]), true));
    }
    ...
    fn component_id_set_remove_out_of_range() {
        ...
        assert!(set.iter().eq([&ComponentId::from_u32(1)]));
    }

    #[test]
    fn component_id_set_is_subset_is_disjoint() {
        let set_1234 = set_from_u32s(vec![1, 2, 3, 4]);
        let set_23 = set_from_u32s(vec![2, 3]);
        let set_45 = set_from_u32s(vec![4, 5]);
    ...
    fn component_id_set_union_intersection_difference() {
        let set_13 = set_from_u32s(vec![1, 3]);
        let set_23 = set_from_u32s(vec![2, 3]);

        assert_eq!(
            set_13.union(&set_23).copied().collect::<ComponentIdSet>(),
            set_from_u32s(vec![1, 2, 3])
        );
        assert_eq!(
            set_23.union(&set_13).copied().collect::<ComponentIdSet>(),
            set_from_u32s(vec![1, 2, 3])
        );
        assert_eq!(
            set_13
