...
impl<T: SparseSetIndex> Access<T> {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            component_writes: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new(),
            resource_read_and_writes: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new(),
            resource_writes: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new(),
            archetypal: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new(),
            marker: PhantomData,
        }
    }
    ...
    pub fn extend(&mut self, other: &Access<T>) {
        ...
        match (
            ...
            (false, true) => {
                self.component_read_and_writes
                    .reverse_difference_with(&other.component_read_and_writes);
            }
            (false, false) => {
                self.component_read_and_writes
        ...
        match (
            ...
            (false, true) => {
                self.component_writes
                    .reverse_difference_with(&other.component_writes);
            }
            (false, false) => {
    ...
    fn get_component_conflicts(&self, other: &Access<T>) -> AccessConflicts {
        let mut conflicts = SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new();
        ...
        for (
            ...
            let temp_conflicts: SortedVecSet<ACCESS_SMALL_VEC_SIZE> =
                match (lhs_writes_inverted, rhs_reads_and_writes_inverted) {
    ...
    pub fn try_iter_component_access(
        ...
        let reads_and_writes = self.component_read_and_writes.iter().map(|index| {
...
impl AccessConflicts {
    ...
    pub(crate) fn empty() -> Self {
        Self::Individual(SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::new())
    }
}
...
impl<T: SparseSetIndex> FilteredAccess<T> {
    ...
    pub fn matches_everything() -> Self {
        Self {
            ...
            required: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::default(),
...
impl<T: SparseSetIndex> Default for AccessFilters<T> {
    fn default() -> Self {
        Self {
            with: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::default(),
            without: SortedVecSet::<ACCESS_SMALL_VEC_SIZE>::default(),
            _index_type: PhantomData,
        }
    }
}
