...
impl<D: QueryData, F: QueryFilter> QueryState<D, F> {
    ...
    ///
    /// assert_eq!(match query_state.get_many(&mut world, [wrong_entity]).unwrap_err() {QueryEntityError::EntityDoesNotExistError(error) => error.entity(), _ => panic!()}, wrong_entity);
    /// ```
    #[inline]
    ...
    /// let wrong_entity = Entity::from_raw_u32(365).unwrap();
    ///
    /// assert_eq!(match query_state.get_many_unique(&mut world, UniqueEntityArray::from([wrong_entity])).unwrap_err() {QueryEntityError::EntityDoesNotExistError(error) => error.entity(), _ => panic!()}, wrong_entity);
    /// ```
    #[inline]
