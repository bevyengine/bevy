...
use crate::component::{CloneBehavior, ComponentCloneFn};
...
pub struct EntityCloner {
    ...
    clone_behavior_overrides: HashMap<ComponentId, CloneBehavior>,
...
impl Default for EntityCloner {
    fn default() -> Self {
        Self {
            ...
            is_recursive: false,
            default_clone_fn: CloneBehavior::global_default_fn(),
...
/// the component will be cloned using the [default cloning strategy](crate::component::CloneBehavior::global_default_fn).
/// To use `Clone`-based handler ([`CloneBehavior::clone`]) in this case it should be set manually using one
...
/// ```
/// # use bevy_ecs::prelude::*;
/// # use bevy_ecs::component::{StorageType, CloneBehavior, Mutable};
/// #[derive(Clone)]
...
///     fn clone_behavior() -> CloneBehavior {
///         CloneBehavior::clone::<Self>()
///     }
/// }
/// ```
///
/// # Handlers
/// `EntityClonerBuilder` clones entities by cloning components using [`CloneBehavior`], and there are multiple layers
...
impl<'w> EntityClonerBuilder<'w> {
    ...
    /// Overrides the [`CloneBehavior`] for a component in this builder.
    ...
    /// Overrides the [`CloneBehavior`] for a component with the given `component_id` in this builder.
    /// This handler will be used to clone the component instead of the global one defined by the [`EntityCloner`].
    ///
    /// See [Handlers section of `EntityClonerBuilder`](EntityClonerBuilder#handlers) to understand how this affects handler priority.
    ...
    /// Removes a previously set override of [`CloneBehavior`] for a component in this builder.
...
mod tests {
    ...
    use crate::{
        self as bevy_ecs,
        component::{CloneBehavior, Component, ComponentDescriptor, StorageType},
        entity::EntityCloner,
    ...
    mod reflect {
        ...
        fn clone_entity_using_reflect() {
            ...
            EntityCloner::build(&mut world)
                .override_clone_behavior::<A>(CloneBehavior::reflect())
        ...
        fn clone_entity_using_reflect_all_paths() {
            ...
            EntityCloner::build(&mut world)
                .override_clone_behavior_with_id(a_id, CloneBehavior::reflect())
                .override_clone_behavior_with_id(b_id, CloneBehavior::reflect())
                .override_clone_behavior_with_id(c_id, CloneBehavior::reflect())
                .clone_entity(e, e_clone);
        ...
        fn read_source_component_reflect_should_return_none_on_invalid_reflect_from_ptr() {
            ...
            EntityCloner::build(&mut world)
                .override_clone_behavior::<A>(CloneBehavior::Custom(test_handler))
                .clone_entity(e, e_clone);
        }
        ...
        fn clone_entity_using_reflect_should_skip_without_panic() {
            ...
            EntityCloner::build(&mut world)
                .override_clone_behavior::<A>(CloneBehavior::reflect())
                .override_clone_behavior::<B>(CloneBehavior::reflect())
                .clone_entity(e, e_clone);
    ...
    fn clone_entity_with_dynamic_components() {
        ...
        let descriptor = unsafe {
            ComponentDescriptor::new_with_layout(
                ...
                None,
                true,
                CloneBehavior::Custom(test_handler),
            )
        };
