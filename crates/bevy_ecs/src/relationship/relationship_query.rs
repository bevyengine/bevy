...
impl<'w, 's, D: QueryData, F: QueryFilter> Query<'w, 's, D, F> {
    ...
    where
        <D as QueryData>::ReadOnly: WorldQuery<Item<'w, 's> = &'w R>,
    {
    ...
    where
        <D as QueryData>::ReadOnly: WorldQuery<Item<'w, 's> = &'w S>,
    {
        self.get(entity)
            .into_iter()
    ...
    where
        D::ReadOnly: WorldQuery<Item<'w, 's> = (Option<&'w R>, Option<&'w R::RelationshipTarget>)>,
    {
        self.get(entity)
            .ok()
    ...
    where
        D::ReadOnly: WorldQuery<Item<'w, 's> = &'w S>,
    {
    ...
    where
        D::ReadOnly: WorldQuery<Item<'w, 's> = &'w R>,
    {
