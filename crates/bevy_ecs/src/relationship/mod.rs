//! This module provides functionality to link entities to each other using specialized components
...
/// A [`Component`] on a "source" [`Entity`] that references another target [`Entity`], creating a
/// "relationship" between them. Every [`Relationship`] has a corresponding [`RelationshipTarget`]
/// type (and vice-versa), which exists on the "target" entity of a relationship and contains the
/// list of all "source" entities that relate to the given "target"
///
/// The [`Relationship`] component is the "source of truth" and the [`RelationshipTarget`] component
/// reflects that source of truth. When a [`Relationship`] component is inserted on an [`Entity`],
/// the corresponding [`RelationshipTarget`] component is immediately inserted on the target
/// component if it does not already exist, and the "source" entity is automatically added to the
/// [`RelationshipTarget`] collection (this is done via "component hooks").
///
/// A common example of a [`Relationship`] is the parent / child relationship. Bevy ECS includes a
/// canonical form of this via the [`ChildOf`](crate::hierarchy::ChildOf) [`Relationship`] and the
/// [`Children`](crate::hierarchy::Children) [`RelationshipTarget`].
///
/// [`Relationship`] and [`RelationshipTarget`] should always be derived via the [`Component`] trait
/// to ensure the hooks are set up properly.
///
/// ## Derive
///
/// [`Relationship`] and [`RelationshipTarget`] can only be derived for structs with a single
/// unnamed field, single named field or for named structs where one field is annotated with
/// `#[relationship]`. If there are additional fields, they must all implement [`Default`].
///
/// [`RelationshipTarget`] also requires that the relationship field is private to prevent direct
/// mutation, ensuring the correctness of relationships.
/// ```
...
/// ```
///
/// When deriving [`RelationshipTarget`] you can specify the `#[relationship_target(linked_spawn)]`
/// attribute to automatically despawn entities stored in an entity's [`RelationshipTarget`] when
...
pub trait Relationship: Component + Sized {
    /// The [`Component`] added to the "target" entities of this [`Relationship`], which contains
    /// the list of all "source" entities that relate to the "target".
    ...
    /// The `on_insert` component hook that maintains the [`Relationship`] / [`RelationshipTarget`]
    /// connection.
    fn on_insert(
    ...
    /// The `on_replace` component hook that maintains the [`Relationship`] / [`RelationshipTarget`]
    /// connection.
                ...
                target_entity_mut.get_mut::<Self::RelationshipTarget>()
            {
                ...
                if relationship_target.len() == 0 {
                    if let Ok(mut entity) = world.commands().get_entity(target_entity) {
                        // this "remove" operation must check emptiness because in the event that an
                        // identical relationship is inserted on top, this
                        // despawn would result in the removal of that identical
...
/// A [`Component`] containing the collection of entities that relate to this [`Entity`] via the
/// associated `Relationship` type. See the [`Relationship`] documentation for more information.
pub trait RelationshipTarget: Component<Mutability = Mutable> + Sized {
    /// If this is true, when despawning or cloning (when [linked cloning is
    /// enabled](crate::entity::EntityClonerBuilder::linked_cloning)), the related entities
    /// targeting this entity will also be despawned or cloned.
    ///
    /// For example, this is set to `true` for Bevy's built-in parent-child relation, defined by
    /// [`ChildOf`](crate::prelude::ChildOf) and [`Children`](crate::prelude::Children).
    /// This means that when a parent is despawned, any children targeting that parent are also
    /// despawned (and the same applies to cloning).
    ///
    /// To get around this behavior, you can first break the relationship between entities, and
    /// *then* despawn or clone. This defaults to false when derived.
    ...
    /// structures that can be used inside of your component. If you need a new collection type,
    /// you can implement the [`RelationshipSourceCollection`] trait for a type you own which
    /// wraps the collection you want to use (to avoid the orphan rule), or open an issue on the
    /// Bevy repository to request first-party support for your collection type.
    ...
    ///
    /// # Warning
    /// This should generally not be called by user code, as modifying the internal collection could
    /// invalidate the relationship. The collection should not contain duplicates.
    fn collection_mut_risky(&mut self) -> &mut Self::Collection;

    /// Creates a new [`RelationshipTarget`] from the given [`RelationshipTarget::Collection`].
    ///
    /// # Warning
    /// This should generally not be called by user code, as constructing the internal collection
    /// could invalidate the relationship. The collection should not contain duplicates.
    ...
    /// The `on_despawn` component hook that despawns entities stored in an entity's
    /// [`RelationshipTarget`] when that entity is despawned.
    // note: think of this as "on_drop"
...
/// when the corresponding [`Relationship`] sources of truth are inserted. Cloning the actual
/// entities in the original [`RelationshipTarget`] would result in duplicates, so we don't do that!
///
/// This will also queue up clones of the relationship sources if the
/// [`EntityCloner`](crate::entity::EntityCloner) is configured to spawn recursively.
