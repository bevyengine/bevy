        ...
        *self.cursor.as_mut() = stop;
        ...
        while local_cursor < stop {
            ...
            if let Err(payload) = result {
                ...
                // recursively in, an applied Command, the correct command order
                // will be retained.
                ...
                *self.cursor.as_mut() = start;
                ...
                // This was the "top of the apply stack". If we are _not_ at the top of the apply
                // stack, when we call`resume_unwind" the caller "closer to the top"
                // will catch the unwind and do this check, until we reach the top.
                if start == 0 {
