...
impl<'w> UnsafeWorldCell<'w> {
    ...
    #[inline]
    pub unsafe fn get_resource<R: Resource>(self, access: impl AsAccess) -> Option<&'w R> {
        ...
        unsafe {
            self.get_resource_by_id(access, component_id)
    ...
    /// - no mutable reference to the resource exists at the same time
    #[inline]
    pub unsafe fn get_resource_ref<R: Resource>(self, access: impl AsAccess) -> Option<Ref<'w, R>> {
        let component_id = self.components().get_valid_id(TypeId::of::<R>())?;

        // SAFETY: caller ensures `self` has permission to access the resource
        // caller also ensures that no mutable reference to the resource exists
        let (ptr, ticks) = unsafe { self.get_resource_with_ticks(access, component_id)? };
    ...
    pub unsafe fn get_resource_by_id(
        ...
        entity_cell.get_by_id(access, component_id)
    }
    ...
    #[inline]
    pub unsafe fn get_resource_mut<R: Resource>(self, access: impl AsAccess) -> Option<Mut<'w, R>> {
        self.assert_allows_mutable_access();
        let component_id = self.components().get_valid_id(TypeId::of::<R>())?;
        // SAFETY:
        ...
        unsafe {
            self.get_resource_mut_by_id(access, component_id)
    ...
    pub unsafe fn get_resource_mut_by_id(
        ...
        let entity = unsafe { self.resource_entities() }.get(component_id)?;
        let entity_cell = self.get_entity(*entity).ok()?;
        entity_cell.get_mut_by_id(access, component_id).ok()
    }
    ...
    pub(crate) unsafe fn get_resource_with_ticks(
        ...
        if !access.has_read(component_id) {
            return None;
        }
    ...
    pub unsafe fn fallback_error_handler(&self) -> ErrorHandler {
        ...
        unsafe { self.get_resource::<FallbackErrorHandler>(All) }
            .copied()
            .unwrap_or_default()
            .0
    }
...
mod tests {
    ...
    fn as_unsafe_world_cell_readonly_resource_mut_forbidden() {
        ...
        let world_cell = world.as_unsafe_world_cell_readonly();
        // SAFETY: this invalid usage will be caught by a runtime panic.
        let _ = unsafe { world_cell.get_resource_mut::<R>(All) };
    }
