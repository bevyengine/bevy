...
fn relation_spawn_raw(storage_type: StorageType) {
    ...
    world
        .register_component(ComponentDescriptor::new_targeted::<ChildOf>(storage_type))
        .unwrap();
...
fn without_filter_raw(storage_type: StorageType) {
    let mut world = World::new();
    world
        .register_component(ComponentDescriptor::new_targeted::<u32>(storage_type))
        .unwrap();
