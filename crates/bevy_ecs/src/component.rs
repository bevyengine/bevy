...
impl ComponentDescriptor {
    ...
    pub fn new_resource<T: Resource>() -> Self {
        Self {
            ...
            drop: needs_drop::<T>().then_some(Self::drop_ptr::<T> as _),
            mutable: true,
            clone_behavior: CloneBehavior::Default,
        }
    }
...
pub enum CloneBehavior {
    /// Uses the default behavior (which is passed to [`CloneBehavior::resolve`])
    #[default]
    Default,
    ...
    /// Uses a [`ComponentCloneFn`] that produces an empty version of the given component.
    // TODO: this exists so that the current scene spawning code can know when to spawn an empty version of this component.
...
impl CloneBehavior {
    ...
    /// Resolves the [`CloneBehavior`] to a [`ComponentCloneFn`]. If [`CloneBehavior::Default`] is
    /// specified, the given `default` function will be used.
    pub fn resolve(&self, default: ComponentCloneFn) -> ComponentCloneFn {
        match self {
            CloneBehavior::Default => default,
            CloneBehavior::Ignore => component_clone_ignore,
            CloneBehavior::Custom(custom) | CloneBehavior::Empty(custom) => *custom,
        }
    }
}
...
pub trait DefaultCloneBehaviorBase {
    fn default_clone_behavior(&self) -> CloneBehavior;
}
