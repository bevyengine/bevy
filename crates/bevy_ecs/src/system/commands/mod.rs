...
impl EntityCommands<'_> {
    ...
    pub unsafe fn insert_by_id<T: Send + 'static>(
        ...
        self.add(unsafe {insert_by_id(component_id, value, move |entity| {
            panic!("{caller} error[B0003]: Could not insert a component {component_id:?} (with type {}) for entity {entity:?} because it doesn't exist in this World. See: https://bevyengine.org/learn/errors/#b0003", std::any::type_name::<T>());
        })});
        self
    }
...
fn insert<T: Bundle>(bundle: T) -> impl EntityCommand {
    ...
    move |entity: Entity, world: &mut World| {
        if let Some(mut entity) = world.get_entity_mut(entity) {
            ...
            panic!("{caller} error[B0003]: Could not insert a bundle (of type `{}`) for entity {:?} because it doesn't exist in this World. See: https://bevyengine.org/learn/errors/b0003", std::any::type_name::<T>(), entity);
        }
    }
}
