...
unsafe impl<'a, T: Resource> SystemParam for Res<'a, T> {
    ...
    fn init_state(world: &mut World, system_meta: &mut SystemMeta) -> Self::State {
        ...
        assert!(!combined_access.has_resource_write(component_id), "error[B0002]: Res<{}> in system {} conflicts with a previous ResMut<{0}> access. Consider removing the duplicate access. See: https://bevyengine.org/learn/errors/b0002", core::any::type_name::<T>(), system_meta.name,);
        system_meta
            .component_access_set
...
unsafe impl<'a, T: Resource> SystemParam for ResMut<'a, T> {
    ...
    fn init_state(world: &mut World, system_meta: &mut SystemMeta) -> Self::State {
        ...
        if combined_access.has_resource_write(component_id) {
            panic!("error[B0002]: ResMut<{}> in system {} conflicts with a previous ResMut<{0}> access. Consider removing the duplicate access. See: https://bevyengine.org/learn/errors/b0002", core::any::type_name::<T>(), system_meta.name);
        } else if combined_access.has_resource_read(component_id) {
            panic!("error[B0002]: ResMut<{}> in system {} conflicts with a previous Res<{0}> access. Consider removing the duplicate access. See: https://bevyengine.org/learn/errors/b0002", core::any::type_name::<T>(), system_meta.name);
        }
        system_meta
            .component_access_set
...
unsafe impl<'a, T: 'static> SystemParam for NonSend<'a, T> {
    ...
    fn init_state(world: &mut World, system_meta: &mut SystemMeta) -> Self::State {
        ...
        let combined_access = system_meta.component_access_set.combined_access();
        assert!(!combined_access.has_resource_write(component_id), "error[B0002]: NonSend<{}> in system {} conflicts with a previous mutable resource access ({0}). Consider removing the duplicate access. See: https://bevyengine.org/learn/errors/b0002", core::any::type_name::<T>(), system_meta.name,);
        system_meta
            .component_access_set
            .add_unfiltered_resource_read(component_id);
