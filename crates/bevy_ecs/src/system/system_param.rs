...
/// system.run((), world);
/// ```
pub struct Local<'a, T: Component, ValueFrom: local_value::LocalValue = local_value::Default> {
    value: &'a mut T,
    value_from: std::marker::PhantomData<ValueFrom>,
}

// SAFE: Local only accesses internal state
unsafe impl<T: Component, ValueFrom: local_value::LocalValue> ReadOnlySystemParamFetch
    for LocalState<T, ValueFrom>
{
}

impl<'a, T: Component, ValueFrom: local_value::LocalValue> Debug for Local<'a, T, ValueFrom>
where
    T: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<'a, T: Component, ValueFrom: local_value::LocalValue> Deref for Local<'a, T, ValueFrom> {
    type Target = T;
...
impl<'a, T: Component, ValueFrom: local_value::LocalValue> DerefMut for Local<'a, T, ValueFrom> {
    #[inline]
...
pub struct LocalState<T: Component, ValueFrom: local_value::LocalValue> {
    value: T,
    value_from: std::marker::PhantomData<ValueFrom>,
}

impl<'a, T: Component + FromWorld> SystemParam for Local<'a, T, local_value::FromWorld> {
    type Fetch = LocalState<T, local_value::FromWorld>;
}
impl<'a, T: Component + Default> SystemParam for Local<'a, T, local_value::Default> {
    type Fetch = LocalState<T, local_value::Default>;
}
impl<'a, T: Component> SystemParam for Local<'a, T, local_value::NeedConfig> {
    type Fetch = LocalState<T, local_value::NeedConfig>;
}

// SAFE: only local state is accessed
unsafe impl<T: Component + FromWorld> SystemParamState for LocalState<T, local_value::FromWorld> {
...
// SAFE: only local state is accessed
unsafe impl<T: Component + Default> SystemParamState for LocalState<T, local_value::Default> {
    type Config = Option<T>;
...
// SAFE: only local state is accessed
unsafe impl<T: Component> SystemParamState for LocalState<T, local_value::NeedConfig> {
    type Config = Option<T>;
...
impl<'a, T: Component + FromWorld> SystemParamFetch<'a> for LocalState<T, local_value::FromWorld> {
    type Item = Local<'a, T, local_value::FromWorld>;
...
impl<'a, T: Component + Default> SystemParamFetch<'a> for LocalState<T, local_value::Default> {
    type Item = Local<'a, T, local_value::Default>;
...
impl<'a, T: Component> SystemParamFetch<'a> for LocalState<T, local_value::NeedConfig> {
    type Item = Local<'a, T, local_value::NeedConfig>;
