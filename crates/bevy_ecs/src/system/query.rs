...
/// |[`many`]\[[`_mut`][`many_mut`]]|Returns the query items for the specified entities.|
...
impl<'w, 's, D: QueryData, F: QueryFilter> Query<'w, 's, D, F> {
    ...
    ///
    /// Deprecated alias for [`many`](Self::many).
    #[inline]
    ...
    pub fn get_many<const N: usize>(
        ...
        // Note that this calls `many_readonly` instead of `many_inner`
        // since we don't need to check for duplicates.
        self.as_readonly().many_readonly(entities)
    }
    ...
    ///
    /// Deprecated alias for [`many_mut`](Self::many_mut).
    #[inline]
    ...
    pub fn get_many_mut<const N: usize>(
        ...
        self.reborrow().many_inner(entities)
    }
    ...
    /// This consumes the [`Query`] to return results with the actual "inner" world lifetime.
    ///
    /// Deprecated alias for [`many_inner`](Self::many_inner).
    #[inline]
    #[deprecated(note = "Use `many_inner` instead, which now returns a Result.")]
    ...
    /// - [`many_readonly`](Self::many_readonly) to get read-only query items without checking for duplicate entities
    ...
    /// - [`many`](Self::many) to get read-only query items without checking for duplicate entities.
    /// - [`many_mut`](Self::many_mut) to get items using a mutable reference.
    /// - [`many_inner`](Self::many_readonly) to get mutable query items with the actual "inner" world lifetime.
    #[inline]
    pub fn many_readonly<const N: usize>(
        self,
        entities: [Entity; N],
    ) -> Result<[D::Item<'w>; N], QueryEntityError<'w>>
    where
    ...
    /// This consumes the [`Query`] to return results with the actual "inner" world lifetime.
    ///
    /// Deprecated alias for [`many_readonly`](Self::many_readonly).
    #[inline]
    #[deprecated(note = "Use `many_readonly` instead, which now returns a Result.")]
...
mod tests {
    ...
    fn many_uniqueness() {
        ...
        // It's best to test many_inner directly, as it is shared
        // We don't care about aliased mutability for the read-only equivalent

        // SAFETY: Query does not access world data.
        assert!(query_state
            .query_mut(&mut world)
            .many_inner::<10>(entities.clone().try_into().unwrap())
            .is_ok());

        assert_eq!(
            query_state
                .query_mut(&mut world)
                .many_inner([entities[0], entities[0]])
                .unwrap_err(),
            QueryEntityError::AliasedMutability(entities[0])
        );
        ...
        assert_eq!(
            query_state
                .query_mut(&mut world)
                .many_inner([entities[0], entities[1], entities[0]])
                .unwrap_err(),
            QueryEntityError::AliasedMutability(entities[0])
        );
        ...
        assert_eq!(
            query_state
                .query_mut(&mut world)
                .many_inner([entities[9], entities[9]])
                .unwrap_err(),
