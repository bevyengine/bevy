...
use quote::quote;
use syn::{parse_macro_input, parse_quote, spanned::Spanned, DeriveInput, Ident, Path};
...
pub fn derive_relationship(input: TokenStream) -> TokenStream {
    ...
    let Some(relationship_sources) = ast
        ...
        return syn::Error::new(
            ast.span(),
            "Relationships must define a relationship_sources(SOURCES_COMPONENT) attribute.",
        )
        .into_compile_error()
        .into();
    };
