...
pub fn derive_gizmo_config_group(input: TokenStream) -> TokenStream {
    ...
    let bevy_gizmos_path: Path = BevyManifest::default_shared().get_path("bevy_gizmos");
    let bevy_reflect_path: Path = BevyManifest::default_shared().get_path("bevy_reflect");
