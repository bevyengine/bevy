...
#[derive(Asset, Debug, Clone, Reflect, PartialEq, Default)]
#[reflect(Clone)]
...
impl MeshExtractableData {
    ...
    pub fn compute_normals(&mut self, topology: PrimitiveTopology) {
        assert!(
            matches!(topology, PrimitiveTopology::TriangleList),
            "`compute_normals` can only work on `TriangleList`s"
        );
        if self.indices().is_none() {
            self.compute_flat_normals(topology);
        } else {
            self.compute_smooth_normals(topology);
        }
    }
    ...
    pub fn compute_flat_normals(&mut self, topology: PrimitiveTopology) {
        assert!(
    ...
    pub fn compute_smooth_normals(&mut self, topology: PrimitiveTopology) {
        self.compute_custom_smooth_normals(
            |[a, b, c], positions, normals| {
    ...
    /// Panics if the mesh has any other topology than [`PrimitiveTopology::TriangleList`].
    /// Panics if the mesh does not have indices defined.
    pub fn compute_area_weighted_normals(&mut self, topology: PrimitiveTopology) {
        self.compute_custom_smooth_normals(
            |[a, b, c], positions, normals| {
    ...
    pub fn generate_tangents(
        &mut self,
        topology: PrimitiveTopology,
    ) -> Result<(), super::GenerateTangentsError> {
        use crate::Mesh;
