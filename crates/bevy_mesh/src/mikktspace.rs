...
/// Correctly scales and renormalizes an already normalized `normal` by the scale determined by its reciprocal `scale_reciprocal`
pub(crate) fn scale_normal(normal: Vec3, scale_reciprocal: Vec3) -> Vec3 {
    // This is basically just `normal * scale_reciprocal` but with the added rule that `0. * anything == 0.`
    // This is necessary because components of `scale_reciprocal` may be infinities, which do not multiply to zero
    let n = Vec3::select(
        normal.cmpeq(Vec3::ZERO),
        Vec3::ZERO,
        normal * scale_reciprocal,
    );

    // If n is finite, no component of `scale_reciprocal` was infinite or the normal was perpendicular to the scale
