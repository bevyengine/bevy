...
mod tests {
    ...
    /// Each motor's intensity must map to its own gilrs effect: `strong_motor`
    /// to [`BaseEffectType::Strong`] and `weak_motor` to [`BaseEffectType::Weak`]
    /// (regression test - the weak motor was previously mapped to `Strong`).
    #[test]
