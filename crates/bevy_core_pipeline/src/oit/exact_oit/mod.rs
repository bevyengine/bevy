//! Order Independent Transparency (OIT) for 3d rendering. See [`ExactOitPlugin`] for more details.
...
use resolve::{
    node::{ExactOitResolveNode, ExactOitResolvePass},
    OitResolvePlugin,
};
...
// ExactOit is also a Component. We explicitly implement the trait so
// we can hook on_add to issue a warning in case `layer_count` is seemingly too high.
impl Component for ExactOit {
    const STORAGE_TYPE: StorageType = StorageType::SparseSet;
    type Mutability = Mutable;

    fn on_add() -> Option<ComponentHook> {
        Some(|world, context| {
            if let Some(value) = world.get::<ExactOit>(context.entity)
                && value.layer_count > 32
            {
                warn!(
                    "{}ExactOit layer_count set to {} might be too high.",
                    context
                        .caller
...
///
/// To enable this for a camera you need to add the [`ExactOit`] component to it and set material alpha mode to `AlphaMode::UnsortedBlend`.
///
...
/// The buffer contains N layers for each pixel, where N can be set with [`ExactOit::layer_count`].
...
pub struct ExactOitPlugin;
impl Plugin for ExactOitPlugin {
    fn build(&self, app: &mut App) {
        load_shader_library!(app, "oit_draw.wgsl");

        app.add_plugins((
            ExtractComponentPlugin::<ExactOit>::default(),
            OitResolvePlugin,
        ))
        ...
        render_app
            ...
            .add_systems(
                Render,
                (
                    configure_depth_texture_usages.in_set(RenderSystems::ManageViews),
                    prepare_oit_buffers.in_set(RenderSystems::PrepareResources),
                ),
            );

        render_app
            .add_render_graph_node::<ViewNodeRunner<ExactOitResolveNode>>(
                Core3d,
                ExactOitResolvePass,
            )
...
fn configure_depth_texture_usages(mut cameras: Query<&mut Camera3d, With<ExactOit>>) {
...
fn check_msaa(cameras: Query<&Msaa, With<ExactOit>>) {
    for msaa in &cameras {
        if msaa.samples() > 1 {
            panic!("MSAA is not supported when using ExactOit");
        }
    }
}
...
pub struct ExactOitBuffers {
    ...
    pub settings: DynamicUniformBuffer<ExactOit>,
}
...
pub fn init_oit_buffers(
    ...
    commands.insert_resource(ExactOitBuffers {
        layers,
        layer_ids,
        settings,
    });
...
pub fn prepare_oit_buffers(
    ...
    render_device: Res<RenderDevice>,
    render_queue: Res<RenderQueue>,
    cameras: Query<(&ExtractedCamera, &ExactOit), (Changed<ExtractedCamera>, Changed<ExactOit>)>,
    camera_oit_uniforms: Query<(Entity, &ExactOit)>,
    mut buffers: ResMut<ExactOitBuffers>,
) {
    ...
    if let Some(mut writer) = buffers.settings.get_writer(
        ...
        for (entity, settings) in &camera_oit_uniforms {
            ...
            commands.entity(entity).insert(ExactOitOffset { offset });
        }
    }
}
