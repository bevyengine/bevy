...
pub struct Camera3d {
    ...
    /// texture copy, so it's recommended to keep this number to a resonably low value. Defaults to
    /// `1`.
    ///
    /// ### Notes
    ///
    ...
    /// [`TemporalAntiAliasPlugin`](crate::experimental::taa::TemporalAntiAliasPlugin).
...
// NOTE: ideally Perspective and Orthographic defaults can share the same impl, but sadly it breaks
