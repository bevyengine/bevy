...
use crate::deferred::{
    DEFERRED_EXTENDED_PREPASS_FORMAT, DEFERRED_LIGHTING_PASS_ID_FORMAT, DEFERRED_PREPASS_FORMAT,
};
...
pub struct ViewPrepassTextures {
    ...
    /// The extended gbuffer generated by the deferred pass.
    /// Exists only if [`DeferredPrepass`] is added to the `ViewTarget`
    pub deferred_extended: Option<ColorAttachment>,
...
impl ViewPrepassTextures {
    ...
    pub fn deferred_extended_view(&self) -> Option<&TextureView> {
        self.deferred_extended
            .as_ref()
            .map(|t| &t.texture.default_view)
    }
}
