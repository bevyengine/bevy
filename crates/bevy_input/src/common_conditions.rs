...
///         .add_systems(Update, pause_menu.run_if(|paused: Res<'_, Paused>| paused.0))
///         .run();
/// }
///
/// fn toggle_pause_state(mut paused: ResMut<'_, Paused>) {
///     paused.0 = !paused.0;
/// }
///
/// fn pause_menu() {
    ...
    T: Copy + Eq + Hash + Send + Sync + 'static,
{
    ...
    move |inputs: Res<'_, ButtonInput<T>>| {
...
pub fn input_pressed<T>(input: T) -> impl FnMut(Res<'_, ButtonInput<T>>) -> bool + Clone
where
    T: Copy + Eq + Hash + Send + Sync + 'static,
{
    move |inputs: Res<'_, ButtonInput<T>>| inputs.pressed(input)
}
...
/// # fn jump() {}
/// ```
pub fn input_just_pressed<T>(input: T) -> impl FnMut(Res<'_, ButtonInput<T>>) -> bool + Clone
where
    T: Copy + Eq + Hash + Send + Sync + 'static,
{
    move |inputs: Res<'_, ButtonInput<T>>| inputs.just_pressed(input)
}

/// Run condition that is active if [`ButtonInput::just_released`] is true for the given input.
pub fn input_just_released<T>(input: T) -> impl FnMut(Res<'_, ButtonInput<T>>) -> bool + Clone
where
    T: Copy + Eq + Hash + Send + Sync + 'static,
{
    move |inputs: Res<'_, ButtonInput<T>>| inputs.just_released(input)
}
