...
use crate::{error::TextError, FontAtlas, FontFamily, FontSmoothing, GlyphAtlasInfo};
...
pub struct FontAtlasSets {
    ...
    pub(crate) sets: HashMap<AssetId<FontFamily>, FontAtlasSet>,
}

impl FontAtlasSets {
    /// Get a reference to the [`FontAtlasSet`] with the given font asset id.
    pub fn get(&self, id: impl Into<AssetId<FontFamily>>) -> Option<&FontAtlasSet> {
        let id: AssetId<FontFamily> = id.into();
        self.sets.get(&id)
    }
    /// Get a mutable reference to the [`FontAtlasSet`] with the given font asset id.
    pub fn get_mut(&mut self, id: impl Into<AssetId<FontFamily>>) -> Option<&mut FontAtlasSet> {
        let id: AssetId<FontFamily> = id.into();
...
pub fn remove_dropped_font_atlas_sets(
    ...
    mut font_events: EventReader<AssetEvent<FontFamily>>,
) {
