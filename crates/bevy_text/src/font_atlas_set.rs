...
pub struct FontAtlasSet(HashMap<FontAtlasKey, (AssetId<Font>, Vec<FontAtlas>)>);
...
impl FontAtlasSet {
    ...
    pub fn has_glyph(&self, cache_key: cosmic_text::CacheKey, font_key: &FontAtlasKey) -> bool {
        self.get(font_key).is_some_and(|(_, font_atlas)| {
...
pub fn free_unused_font_atlases_system(
    ...
    for event in font_events.read() {
        if let AssetEvent::Removed { id } = event {
            font_atlas_sets.retain(|_, (asset_id, _)| *asset_id != *id);
        }
    }
}
