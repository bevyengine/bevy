...
/// An async executor.
/// Based on the reference executor provided by [async_executor].
///
...
impl<'a> Executor<'a> {
    ...
    unsafe fn spawn_inner<T: 'a>(
        ...
        let future = async move {
            let _guard = OnDrop::new(move || drop(state.active.lock().try_remove(index)));
            future.await
        };
        ...
        let (runnable, task) = unsafe {
            ...
            builder.spawn_unchecked(|()| future, self.schedule())
        };
    ...
    fn state_ptr(&self) -> *const State {
        ...
        if ptr.is_null() {
            ptr = State::alloc_atomic(&self.state);
        }
        ptr
    }
...
pub struct LocalExecutor<'a> {
    ...
    _marker: PhantomData<alloc::rc::Rc<()>>,
}
...
impl State {
    ...
    #[cold]
    fn alloc_atomic(atomic_ptr: &AtomicPtr<Self>) -> *mut Self {
