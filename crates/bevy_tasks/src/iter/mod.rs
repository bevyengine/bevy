    ...
    Self: Sized + Send,
{
        ...
        BatchIter::Item: Send + 'static,
    {
        ...
        pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.partition::<Vec<_>, F>(new_f) });
            }
        })
        .into_iter()
        .for_each(|(c, d)| {
        ...
        F: FnMut(BatchIter::Item) -> bool + Send + Sync + Clone,
    {
        pool.scope(|s| {
            while let Some(mut batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.all(new_f) });
            }
        })
        .into_iter()
        ...
        F: FnMut(BatchIter::Item) -> bool + Send + Sync + Clone,
    {
        pool.scope(|s| {
            while let Some(mut batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.any(new_f) });
            }
        })
        .into_iter()
        ...
        BatchIter::Item: Send + 'static,
    {
        pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.max_by_key(new_f) });
            }
        })
        .into_iter()
        .flatten()
        ...
        BatchIter::Item: Send + 'static,
    {
        pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.max_by(new_f) });
            }
        })
        .into_iter()
        .flatten()
        ...
        BatchIter::Item: Send + 'static,
    {
        pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.min_by_key(new_f) });
            }
        })
        .into_iter()
        .flatten()
        ...
        BatchIter::Item: Send + 'static,
    {
        pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let new_f = f.clone();
                s.spawn(async move { batch.min_by(new_f) });
            }
        })
        .into_iter()
        .flatten()
