...
/// [reflection]: crate
pub trait Map: PartialReflect + Send + Sync {
...
impl<K: Reflect + Send + Sync, V: Reflect + Send + Sync> FromIterator<(K, V)> for DynamicMap {
...
/// println!("{:#?}", &my_map as &(dyn Reflect + Send + Sync));
///
/// // Output:
///
/// // {
...
#[inline]
pub fn map_apply<M: Map>(a: &mut M, b: &(dyn PartialReflect + Send + Sync)) {
