...
/// };
///
/// // Convert the closure into a dynamic closure using `IntoCallable::into_callable`
/// let mut func: DynamicCallable = punctuate.into_callable();
///
...
impl<'env> DynamicCallable<'env> {
    ...
    ///
    /// For [`DynamicCallable`] created using [`IntoCallable`],
    ...
    ///
    /// If this [`DynamicCallable`] was created using [`IntoCallable`],
...
mod tests {
    ...
    #[test]
    fn should_convert_dynamic_closure_with_into_callable() {
        fn make_closure<'env, F: IntoCallable<'env, M>, M>(f: F) -> DynamicCallable<'env> {
            f.into_callable()
        }

        let c = 23;
        let closure: DynamicCallable = make_closure(|a: i32, b: i32| a + b + c);
        let _: DynamicCallable = make_closure(closure);
    }
