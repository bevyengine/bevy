...
impl<'env> DynamicFunctionMut<'env> {
    ...
    /// This info is required in order for [function overloading] to work correctly.
    /// It may also be used by consumers of this function for validation and debugging.
    ///
    /// # Panics
    ///
    ...
    pub fn call<'a>(&mut self, args: ArgList<'a>) -> FunctionResult<'a> {
        match self.function_map {
            FunctionMap::Standard(ref mut func) => func(args),
    ...
    /// assert_eq!(count, 5);
    /// ```
    pub fn call_once(self, args: ArgList) -> FunctionResult {
        match self.function_map {
            FunctionMap::Standard(mut func) => func(args),
            FunctionMap::Overloaded(mut map) => {
                let sig = ArgumentSignature::from(&args);
