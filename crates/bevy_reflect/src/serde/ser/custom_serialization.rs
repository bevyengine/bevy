...
use crate::serde::ReflectSerializeReflect;
...
/// or [`ReflectSerializeReflect`] type data.
///
...
pub(super) fn try_custom_serialize<S: Serializer>(
    ...
    } else if let Some(reflect_serialize_reflect) = registration.data::<ReflectSerializeReflect>() {
        #[cfg(feature = "debug_stack")]
        TYPE_INFO_STACK.with_borrow_mut(crate::type_info_stack::TypeInfoStack::pop);

        Ok(reflect_serialize_reflect.serialize(value, serializer, type_registry))
    } else {
        Err((serializer, make_custom_error(format_args!(
            "type `{}` did not register the `ReflectSerialize` or `ReflectSerializeReflect` type data. For certain types, this may need to be registered manually using `register_type_data`",
            info.type_path(),
        ))))
    }
}
