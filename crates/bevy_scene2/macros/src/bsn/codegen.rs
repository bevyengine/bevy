        ...
        for bsn_entry in &self.entries {
            entries.push(match bsn_entry {
                ...
                BsnEntry::FromTemplateConstructor(BsnConstructor {
                    ...
                    quote! {
                        <#type_path as #bevy_scene::PatchFromTemplate>::patch(
                            move |value, _context| {
                                *value = <#type_path as #bevy_ecs::template::FromTemplate>
                ...
                BsnEntry::InheritedScene(inherited_scene) => match inherited_scene {
                    ...
                    BsnInheritedScene::Fn { function, args } => {
                        quote! {#bevy_scene::SceneScope(#function(#args))}
                    }
                },
                BsnEntry::Name(ident) => {
                ...
                BsnEntry::NameExpression(expr_tokens) => {
                    quote! {
                        <#bevy_ecs::name::Name as #bevy_scene::PatchFromTemplate>::patch(
                            move |value, _context| {
            ...
            match &self.fields {
                BsnFields::Named(fields) => {
                    ...
                    assignments.push(quote! {
                        if !matches!(#(#field_path).*, #bevy_scene::macro_utils::PathResolveHelper::<<#path as #bevy_ecs::template::FromTemplate>::Template>::#variant { .. }) {
                            #maybe_deref #(#field_path).* = #bevy_scene::macro_utils::PathResolveHelper::<<#path as #bevy_ecs::template::FromTemplate>::Template>::#variant_default_name();
                        }
                        if let #bevy_scene::macro_utils::PathResolveHelper::<<#path as #bevy_ecs::template::FromTemplate>::Template>::#variant { #(#field_names, )*.. } = #maybe_borrow_mut #(#field_path).* {
                            #(#field_assignments)*
                        }
                    })
                }
                            ...
                            Some(BsnValue::Name(ident)) => {
                                ...
                                assignments.push(quote!{
                                    #(#field_path.)*#field_name = #bevy_ecs::template::EntityReference::ScopedEntityIndex(
                                        #bevy_ecs::template::ScopedEntityIndex { scope: _context.current_entity_scope(), index: #index }
                                    );
                                })
                            }
                ...
                BsnFields::Tuple(fields) => {
                    for (index, field) in fields.iter().enumerate() {
                        ...
                        match field_value {
                            field_value_type!() => {
                                assignments
                                    .push(quote! {#(#field_path.)*#field_index = #field_value;});
                            }
                            BsnValue::Name(ident) => {
                                let name = ident.to_string();
                                let index = entity_refs.get(name);
...
impl ToTokens for BsnType {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let path = &self.path;
        let maybe_variant = self.enum_variant.as_ref().map(|variant| {
            quote! {::#variant}
        });
