...
impl GltfLoader {
    ...
    pub async fn load_gltf<'a, 'b, 'c>(
        ...
        // Let extension handlers take over the loading of individual textures.
        // Handlers run sequentially because they get exclusive access to the
        // load context; textures they bypass go through the default loading.
        let mut loaded_images: Vec<Option<ImageOrPath>> = Vec::new();
        let mut bypassed_textures = Vec::new();
        for texture in gltf.textures() {
            let is_srgb = !linear_textures.contains(&texture.index());
            let sampler = if settings.override_sampler {
                default_sampler.clone()
            } else {
                texture_sampler(&texture, default_sampler)
            };
            // an Image that can be generated by a user's extension,
            // such as when transcoding KHR_texture_basisu textures
            let mut user_image: Option<Image> = None;
            for extension in extensions.iter_mut() {
                extension
                    .on_texture_load(
                        load_context,
            ...
            match user_image {
                Some(image) => loaded_images.push(Some(ImageOrPath::Image {
                    image,
                    label: GltfAssetLabel::Texture(texture.index()),
                })),
                None => {
                    bypassed_textures.push((loaded_images.len(), texture));
                    loaded_images.push(None);
                }
            }
        }
        // Load the bypassed textures with the default loading.
        ...
        if bypassed_textures.len() == 1 || cfg!(target_arch = "wasm32") {
            for (position, texture) in bypassed_textures {
                loaded_images[position] = Some(
                    load_image(
                        texture,
                        &buffer_data,
                        &linear_textures,
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                let results = IoTaskPool::get().scope(|scope| {
                    bypassed_textures.iter().for_each(|(_, gltf_texture)| {
                ...
                for ((position, _), result) in bypassed_textures.iter().zip(results) {
                    loaded_images[*position] = Some(result?);
                }
            }
        }
        for (image, texture) in loaded_images.into_iter().zip(gltf.textures()) {
            image
                .expect(
                    "all textures are loaded by either an extension handler or the default loading",
                )
                .process_loaded_texture(load_context, &mut texture_handles);
...
"#,
    ...
    #[test]
    fn gltf_extension_texture_load_hook() {
        use alloc::sync::Arc;
        ...
        // Mimics a texture extension handler like `KHR_texture_basisu` would
        // use: it claims only the textures carrying its extension, which may
        // omit the standard `texture.source`, and fabricates their images.
        #[derive(Clone, Default)]
        struct TextureLoadHook(Arc<AtomicUsize>);

        impl GltfExtensionHandler for TextureLoadHook {
            ...
            async fn on_texture_load(
                ...
                _buffer_data: &[Vec<u8>],
                _gltf_path: &AssetPath<'_>,
                _is_srgb: bool,
                _sampler: ImageSamplerDescriptor,
                _supported_compressed_formats: CompressedImageFormats,
                _render_asset_usages: RenderAssetUsages,
                user_image: &mut Option<Image>,
            ) {
                if gltf_texture.extension_value("TEST_texture_load").is_none() {
                    return;
                }
                self.0.fetch_add(1, Ordering::SeqCst);
                *user_image = Some(Image::new_fill(
                    Extent3d {
                        width: 1,
                        height: 1,
        ...
        // A handler that never loads anything must not interfere; the next
        // handler still gets a chance to load the texture.
        #[derive(Clone, Default)]
        struct BypassHook(Arc<AtomicUsize>);

        impl GltfExtensionHandler for BypassHook {
            fn dyn_clone(&self) -> Box<dyn ErasedGltfExtensionHandler> {
                Box::new(self.clone())
            }
            ...
            r#"
{
    ...
    "nodes": [
        {
            "name": "TestTextureLoadHook"
        }
    ],
    "scene": 0,
    "scenes": [{ "nodes": [0] }],
    "textures": [
        { "extensions": { "TEST_texture_load": { "source": 0 } } },
        { "extensions": { "TEST_texture_load": { "source": 0 } } }
    ],
    "materials": [
...
"#,
        ...
        let bypass_hook = BypassHook::default();
        let bypass_count = bypass_hook.0.clone();
        let load_hook = TextureLoadHook::default();
        let load_count = load_hook.0.clone();
        {
            let handlers = app.world().resource::<GltfExtensionHandlers>().0.clone();
            let mut handlers = handlers.write_blocking();
            handlers.push(Box::new(bypass_hook));
            handlers.push(Box::new(load_hook));
        }
        ...
        // keep the handle alive so the loaded assets are not unloaded
        let _handle = handle;

        assert_eq!(bypass_count.load(Ordering::SeqCst), 2);
        assert_eq!(load_count.load(Ordering::SeqCst), 2);

        let gltf_root_assets = app.world().resource::<Assets<Gltf>>();
        let gltf_root = gltf_root_assets.get(&_handle).unwrap();
        let materials = app.world().resource::<Assets<GltfMaterial>>();
        let images = app.world().resource::<Assets<Image>>();
        let red_textures = gltf_root
            .materials
            .iter()
            .map(|handle| materials.get(handle).unwrap())
            .filter_map(|material| material.base_color_texture.as_ref())
            .filter(|handle| {
                images.get(*handle).and_then(|image| image.data.as_deref())
                    == Some(&[255, 0, 0, 255])
            })
            .count();
        assert_eq!(red_textures, 2, "both textures use the hook-provided image");
    }
}
