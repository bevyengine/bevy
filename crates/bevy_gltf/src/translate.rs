...
use crate::{GltfMaterial, GltfAssetLabel};
...
pub struct GltfMaterialTranslator {
    ...
    pub load_material: Arc<dyn Fn(&GltfMaterial, &GltfAssetLabel, &mut LoadContext) -> Result<UntypedHandle, BevyError> + Send + Sync>,
    /// Insert a material component using a label that was previously passed to `load_material`.
    pub insert_material: Arc<dyn Fn(&GltfAssetLabel, &mut LoadContext, &mut EntityWorldMut) -> Result<(), BevyError> + Send + Sync>,
}
