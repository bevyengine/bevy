...
pub trait StateScopedEventsAppExt {
    ...
    ///
    /// Note that event cleanup is ordered ambiguously relative to
    /// [`StateScoped`](crate::prelude::StateScoped) entity cleanup and the [`OnExit`] schedule
    /// for the target state. All of these (state scoped entities and events cleanup, and
    /// `OnExit`) occur within schedule [`StateTransition`](crate::prelude::StateTransition) and
    /// system set `StateTransitionSteps::ExitSchedules`.
