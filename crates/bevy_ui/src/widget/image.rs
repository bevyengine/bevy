use crate::{ContentSize, Measure, MeasureArgs, Node, NodeMeasure, ResolvedScaleFactor};
...
pub fn update_image_content_size_system(
    ...
    for (mut content_size, image, mut image_size, scale_factor) in &mut query {
                ...
                })
        {
            ...
            if size != image_size.size || scale_factor.is_changed() || content_size.is_added() {
                image_size.size = size;
                content_size.set(NodeMeasure::Image(ImageMeasure {
                    // multiply the image size by the scale factor to get the physical size
                    size: size.as_vec2() * scale_factor.0,
                }));
            }
        }
    }
