...
pub fn compute_color_stops(
    mut color_stops_query: Query<(
        &LinearGradient,
        &ColorStops,
        &mut ComputedColorStops,
        &ComputedNode,
        &ComputedNodeTarget,
    )>,
) {
    for (gradient, stops, mut computed_stops, node, target) in color_stops_query.iter_mut() {
        let (start, end) = gradient.gradient_line_endpoints(node.size.x, node.size.y);
        let d = start.distance(end);
        let stops = compute_stops(&stops.0, d, target.logical_size(), target.scale_factor());
...
fn compute_stops(
    ...
    let logical_length = length / scale_factor;
    let logical_viewport_size = viewport_size / scale_factor;
