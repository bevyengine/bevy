...
pub struct UiImage {
    ...
    /// the full image. This is an easy one-off alternative to using a
    /// [`TextureAtlas`](bevy_sprite::TextureAtlas).
    ///
...
/// Ensures this [`Node`]'s front-to-back ordering is not controlled solely by its location in the
/// UI hierarchy.
///
/// A node with a higher z-index will appear on top
...
/// Used to add rounded corners to a UI node.
///
/// You can set a UI node to have uniformly
