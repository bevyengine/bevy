...
pub fn from_style(scale_factor: f64, style: &Style) -> taffy::style::Style {
    taffy::style::Style {
        ...
        grid_template_rows: style.grid_template_rows.iter().map(|item| (*item).into()).collect::<Vec<_>>(),
        grid_template_columns: style.grid_template_columns.iter().map(|item| (*item).into()).collect::<Vec<_>>(),
        grid_auto_rows: style.grid_auto_rows.iter().map(|item| (*item).into()).collect::<Vec<_>>(),
        grid_auto_columns: style.grid_auto_columns.iter().map(|item| (*item).into()).collect::<Vec<_>>(),
