...
impl RectBytesMut<'_> {
    ...
    ///
    /// Will fail if the rects are different sizes or formats. Completely
    /// ignores transparency and will directly overwrite the colors.
...
pub enum TextureCopyError {
    #[error("cannot copy rect of size {src:?} to rect of size {dest:?}")]
    SizeMismatch { src: UVec2, dest: UVec2 },
    #[error("cannot convert from format {src:?} to format {dest:?}")]
    FormatMismatch {
        src: TextureFormat,
        dest: TextureFormat,
    },
