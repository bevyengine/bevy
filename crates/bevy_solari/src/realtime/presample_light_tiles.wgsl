...
#import bevy_solari::sampling::{select_random_light, sample_light, LightSample}
...
fn presample_light_tiles(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(local_invocation_index) sample_index: u32) {
    ...
    let sample = sample_light(&rng, light);

    let i = (tile_id * 1024u) + sample_index;
    light_tile_samples[i] = pack_light_sample(sample);
}

fn pack_light_sample(sample: LightSample) -> LightSamplePacked {
...
fn unpack_light_sample(packed: LightSamplePacked) -> LightSample {
    return LightSample(
        packed.light_id,
        ...
        rgb9e5_to_vec3_(packed.radiance) / view.exposure,
