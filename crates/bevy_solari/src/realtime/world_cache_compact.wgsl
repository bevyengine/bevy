...
#import bevy_solari::realtime_bindings::{
    world_cache,
    world_cache_b,
    set_world_cache_active_cells_count,
}
...
fn decay_world_cache(@builtin(global_invocation_id) global_id: vec3<u32>) {
    var life = world_cache[global_id.x].life;
    if life > 0u {
        life -= 1u;
        world_cache[global_id.x].life = life;

        if life == 0u {
            world_cache[global_id.x].checksums = WORLD_CACHE_EMPTY_CELL;
            world_cache[global_id.x].radiance = vec4(0.0);
            world_cache[global_id.x].luminance_deltas = 0.0;
        }
    }
}
...
fn compact_world_cache_single_block(
    ...
    if t == 0u { w1[0u] = 0u; } else { w1[t] = u32(world_cache[cell_id.x - 1u].life != 0u); }; workgroupBarrier();
    ...
    if t < 512u { world_cache[cell_id.x].a = w2[t]; } else { world_cache[cell_id.x].a = w2[t] + w2[t - 512u]; }
}

@compute @workgroup_size(1024, 1, 1)
fn compact_world_cache_blocks(@builtin(local_invocation_index) t: u32) {
    if t == 0u { w1[0u] = 0u; } else { w1[t] = world_cache[t * 1024u - 1u].a; }; workgroupBarrier();
    if t < 1u { w2[t] = w1[t]; } else { w2[t] = w1[t] + w1[t - 1u]; } workgroupBarrier();
    if t < 2u { w1[t] = w2[t]; } else { w1[t] = w2[t] + w2[t - 2u]; } workgroupBarrier();
    if t < 4u { w2[t] = w1[t]; } else { w2[t] = w1[t] + w1[t - 4u]; } workgroupBarrier();
    if t < 8u { w1[t] = w2[t]; } else { w1[t] = w2[t] + w2[t - 8u]; } workgroupBarrier();
...
fn compact_world_cache_write_active_cells(
    ...
    let compacted_index = world_cache[cell_id.x].a + world_cache_b[workgroup_id.x];
    let cell_active = world_cache[cell_id.x].life != 0u;

    if cell_active {
        world_cache[compacted_index].active_cell_indices = cell_id.x;
    }

    if thread_index == 1023u && workgroup_id.x == 1023u {
        let active_cell_count = compacted_index + u32(cell_active);
        set_world_cache_active_cells_count(active_cell_count);
