...
#import bevy_solari::sampling::{generate_random_light_sample, calculate_resolved_light_contribution, trace_light_visibility}
...
const DIRECT_LIGHT_SAMPLE_COUNT: u32 = 4u;
...
fn sample_radiance(@builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    if active_cell_id.x < world_cache_active_cells_count {
        ...
        var new_radiance = sample_random_light_ris(geometry_data.world_position, geometry_data.world_normal, &rng);
...
fn sample_random_light_ris(world_position: vec3<f32>, world_normal: vec3<f32>, rng: ptr<function, u32>) -> vec3<f32> {
    ...
    for (var i = 0u; i < DIRECT_LIGHT_SAMPLE_COUNT; i++) {
        ...
        let light_contribution = calculate_resolved_light_contribution(sample.resolved_light_sample, world_position, world_normal);
        ...
        if rand_f(rng) < resampling_weight / weight_sum {
            ...
            selected_sample_world_position = sample.resolved_light_sample.world_position;
        }
    }
