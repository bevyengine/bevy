...
#import bevy_solari::realtime_bindings::{light_tile_resolved_samples, view, constants, world_cache}
...
fn sample_di(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    ...
    if rand_f(&rng) >= f32(WORLD_CACHE_CELL_UPDATES_SOFT_CAP) / f32(world_cache.active_cells_count) { return; }
