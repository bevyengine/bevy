...
#import bevy_solari::realtime_bindings::{
    ...
    view,
    constants,
    get_world_cache_active_cells_count,
    world_cache,
}

@compute @workgroup_size(64, 1, 1)
fn sample_di(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    if active_cell_id.x >= get_world_cache_active_cells_count() { return; }

    let cell_index = world_cache[active_cell_id.x].active_cell_indices;
    let geometry_data = world_cache[cell_index].geometry_data;
    var rng = cell_index + constants.frame_index;

    if rand_f(&rng) >= f32(WORLD_CACHE_CELL_UPDATES_SOFT_CAP) / f32(get_world_cache_active_cells_count()) { return; }

    let new_radiance = sample_random_light_ris(geometry_data.world_position, geometry_data.world_normal, workgroup_id.xy, &rng);

    world_cache[active_cell_id.x].active_cells_new_radiance = new_radiance;
}
...
fn sample_gi(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    ...
    if ray.kind != RAY_QUERY_INTERSECTION_NONE {
        ...
        let cell_life = atomicLoad(&world_cache[cell_index].life);
        ...
        world_cache[active_cell_id.x].active_cells_new_radiance += ray_hit.material.base_color * radiance;
    }
}
...
fn blend_new_samples(@builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    ...
    let old_radiance = world_cache[cell_index].radiance;
    let new_radiance = world_cache[active_cell_id.x].active_cells_new_radiance;
    let luminance_delta = world_cache[cell_index].luminance_deltas;
    ...
    world_cache[cell_index].radiance = vec4(blended_radiance, sample_count);
    world_cache[cell_index].luminance_deltas = blended_luminance_delta;
}
