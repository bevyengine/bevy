...
impl Main {
    ...
    pub fn run_main(world: &mut World, mut run_at_least_once: Local<'_, bool>) {
        if !*run_at_least_once {
            world.resource_scope(|world, order: Mut<'_, MainScheduleOrder>| {
...
impl FixedMain {
    ...
    pub fn run_fixed_main(world: &mut World) {
        world.resource_scope(|world, order: Mut<'_, FixedMainScheduleOrder>| {
            for &label in &order.labels {
                let _ = world.try_run_schedule(label);
            }
        });
