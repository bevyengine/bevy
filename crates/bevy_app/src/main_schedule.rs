...
/// input OS events into an `Events` resource. This enables systems in [`Update`] to consume the
/// events from the `Events` resource without actually knowing about (or taking a direct scheduler
/// dependency on) the "os-level keyboard event system".
///
...
/// Runs the [`FixedMain`] schedule in a loop until all relevant elapsed time has been "consumed".
///
...
/// to "global" absolute transforms. This enables the [`PostUpdate`] transform-sync system to react
/// to "local transform" changes in [`Update`] without the [`Update`] systems needing to know about
/// (or add scheduler dependencies for) the "global transform sync system".
///
...
/// but before or after the fixed update logic.
///
/// Systems in this set
...
pub enum RunFixedMainLoopSystem {
    ...
    /// Use this system instead to order your systems to run specifically inbetween the fixed
    /// update logic and all other systems that run in
