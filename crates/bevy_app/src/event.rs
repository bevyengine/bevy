...
pub struct EventReaderConfig<last_event_count, events> {
    pub last_event_count: last_event_count,
    pub events: events,
}
#[allow(non_camel_case_types)]
pub struct EventReaderState<last_event_count, events, T> {
    last_event_count: last_event_count,
    events: events,
    _phantom: std::marker::PhantomData<fn() -> (T,)>,
}
impl<'a, T: bevy_ecs::Resource> bevy_ecs::SystemParam for EventReader<'a, T> {
    type Config = EventReaderConfig<
        <Local<'a, (usize, PhantomData<T>)> as bevy_ecs::SystemParam>::Config,
        <Res<'a, Events<T>> as bevy_ecs::SystemParam>::Config,
    >;
    type State = EventReaderState<
        <Local<'a, (usize, PhantomData<T>)> as bevy_ecs::SystemParam>::State,
        <Res<'a, Events<T>> as bevy_ecs::SystemParam>::State,
        T,
    >;
    fn default_config() -> Self::Config {
        EventReaderConfig {
            last_event_count:
                <Local<'a, (usize, PhantomData<T>)> as bevy_ecs::SystemParam>::default_config(),
            events: <Res<'a, Events<T>> as bevy_ecs::SystemParam>::default_config(),
        }
    }
    fn create_state(config: Self::Config, resources: &mut bevy_ecs::Resources) -> Self::State {
        EventReaderState {
            last_event_count:
                <Local<'a, (usize, PhantomData<T>)> as bevy_ecs::SystemParam>::create_state(
                    config.last_event_count,
                    resources,
                ),
            events: <Res<'a, Events<T>> as bevy_ecs::SystemParam>::create_state(
                config.events,
                resources,
            ),
...
impl<
        'a,
        last_event_count: bevy_ecs::ParamState<'a, Item = Local<'a, (usize, PhantomData<T>)>>,
        events: bevy_ecs::ParamState<'a, Item = Res<'a, Events<T>>>,
        T: bevy_ecs::Resource,
    > bevy_ecs::ParamState<'a> for EventReaderState<last_event_count, events, T>
{
    ...
    fn init(
        ...
        self.last_event_count.init(system_state, world, resources);
        self.events.init(system_state, world, resources);
    }
    unsafe fn get_param(
        &'a mut self,
        system_state: &'a bevy_ecs::SystemState,
        world: &'a bevy_ecs::World,
        resources: &'a bevy_ecs::Resources,
    ) -> Option<Self::Item> {
        Some(EventReader {
            last_event_count: self
                .last_event_count
                .get_param(system_state, world, resources)?,
            events: self.events.get_param(system_state, world, resources)?,
        })
    }
}
