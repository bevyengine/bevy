...
pub fn extract_debug_overlay(
    ...
    mut extracted_ui_layers: ResMut<ExtractedUiLayers>,
    ...
    for (entity, uinode, transform, visibility, maybe_clip) in &uinode_query {
        if !debug_options.show_hidden && !visibility.get() {
            continue;
        }

        // Extract a border box to display an outline for every UI Node in the layout
        extracted_ui_layers.layers[uinode.layer_index as usize]
            .uinodes
