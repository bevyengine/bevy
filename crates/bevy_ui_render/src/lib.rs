...
pub fn extract_text_shadows(
        ...
        &uinode_query
    {
        ...
        let shadow_transform =
            **transform * Affine2::from_translation(shadow.offset / uinode.inverse_scale_factor());
        ...
        ) in text_layout_info.glyphs.iter().enumerate()
        {
            let rect = texture_atlases
                ...
                .as_rect();
            extracted_uinodes.glyphs.push(ExtractedGlyph {
                transform: shadow_transform * Affine2::from_translation(*position),
                rect,
            });
        ...
        &uinode_query
    {
        ...
        for &(section_entity, rect) in text_layout_info.section_rects.iter() {
            ...
            extracted_uinodes.uinodes.push(ExtractedUiNode {
                ...
                item: ExtractedUiItem::Node {
                    atlas_scaling: None,
                    transform: **global_transform * Affine2::from_translation(rect.center()),
                    flip_x: false,
                    flip_y: false,
                    border: uinode.border(),
                    border_radius: uinode.border_radius(),
