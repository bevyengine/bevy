...
fn prepare_bloom_textures(
    ...
    for (entity, camera, bloom) in &views {
        ...
        let mip_height_ratio = if camera.main_texture_size.y != 0 {
            bloom.max_mip_dimension as f32 / camera.main_texture_size.y as f32
        } else {
            0.
        };
        ...
        let texture_descriptor = TextureDescriptor {
            ...
            size: (camera.main_texture_size.as_vec2() * mip_height_ratio)
                .round()
                .as_uvec2()
                .max(UVec2::ONE)
                .to_extents(),
