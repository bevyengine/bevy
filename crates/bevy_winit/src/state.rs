...
impl ApplicationHandler<WinitUserEvent> for WinitAppRunnerState {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        create_windows(
            event_loop,
            create_window.try_get_mut(self.world_mut()).unwrap(),
        );
        ...
        WINIT_WINDOWS.with_borrow(|winit_windows| {
            ACCESS_KIT_ADAPTERS.with_borrow_mut(|access_kit_adapters| {
                let (
                    ...
                    .message_writer_system_state
                    .try_get_mut(self.app.world_mut())
                    .unwrap();
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        create_monitors(
            event_loop,
            create_monitor.try_get_mut(self.world_mut()).unwrap(),
        );
...
impl WinitAppRunnerState {
    fn redraw_requested(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let (config, windows) = focused_windows_state.try_get(self.world()).unwrap();
        ...
        if should_update {
            let (_, windows) = focused_windows_state.try_get(self.world()).unwrap();
