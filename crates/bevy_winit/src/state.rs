...
use bevy_app::{App, AppExit, PluginsState, PluginState};
...
impl<T: Event> ApplicationHandler<T> for WinitAppRunnerState<T> {
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: StartCause) {
        ...
        if self.app.plugins_state() != PluginsState::Done {
            println!("State from new_events: {:?}", self.app.plugins_state());
            self.app.update_plugins();
