...
use super::entity_list::{
    EntityCache, EntityListContainer, EntityListVirtualContent, EntityListVirtualState,
...
pub struct VirtualScrollState {
    /// Target scroll position for smooth scrolling interpolation
    pub target_scroll: f32,
    /// Current actual scroll position in pixels
    pub current_scroll: f32,
    /// Velocity of scrolling for momentum-based animation
    ...
    /// Sorted list of all entity IDs for consistent ordering
    pub sorted_entity_ids: Vec<u32>,
    /// Total number of entities available
    pub total_entity_count: usize,
    /// Total height of all content if fully rendered
    pub total_content_height: f32,
    /// Distance from edge before loading more entities
    pub loading_threshold: f32,
    /// Whether more entities are currently being loaded
    pub is_loading_more: bool,
    /// Number of entities to load per batch
    pub page_size: usize,
    ...
    /// Minimum time between updates to prevent performance issues
    pub min_update_interval: f64,
    /// Maximum allowed scroll velocity to prevent black screens
    pub max_scroll_velocity: f32,
    /// Pending scroll position to apply on next frame
    ...
    /// Timer for throttling loading operations
...
pub struct CachedEntityItem {
    /// The entity ID this cached item represents
    pub entity_id: u32,
    /// Whether this item is currently visible in the viewport
    pub is_visible: bool,
    /// Cached Y position of this item in the virtual list
...
pub fn update_infinite_scrolling_display(
    ...
    mut virtual_content_query: Query<
        (Entity, &mut Node),
        (
            With<EntityListVirtualContent>,
            Without<EntityListContainer>,
            Without<CachedEntityItem>,
        ),
    >,
    container_query: Query<
        &Node,
        (
            With<EntityListContainer>,
            Without<CachedEntityItem>,
            Without<EntityListVirtualContent>,
        ),
    >,
    mut cached_items: Query<
        (Entity, &mut Node, &mut CachedEntityItem, &mut Visibility),
        (
            With<CachedEntityItem>,
            Without<EntityListContainer>,
            Without<EntityListVirtualContent>,
        ),
    >,
    ...
    let Ok((virtual_content, mut virtual_content_node)) = virtual_content_query.single_mut() else {
        return;
    };
    ...
    let scroll_changed = (virtual_scroll_state.current_scroll - current_scroll_y).abs()
        > virtual_scroll_state.item_height * 0.5;
    ...
    let target_scroll_y =
        if let Some(pending_pos) = virtual_scroll_state.pending_scroll_position.take() {
            pending_pos
        } else {
            current_scroll_y
        };
    ...
    let scroll_changed = (virtual_scroll_state.current_scroll - target_scroll_y).abs()
        > virtual_scroll_state.item_height * 0.5;
    ...
    let items_per_screen =
        (virtual_scroll_state.container_height / virtual_scroll_state.item_height).ceil() as usize;
    let scroll_offset =
        (virtual_scroll_state.current_scroll / virtual_scroll_state.item_height) as usize;
    ...
    if range_size > max_virtual_items
        || virtual_scroll_state.total_entity_count > items_per_screen * 2
    {
    ...
    for (entity, _node, mut cached_item, mut visibility) in cached_items.iter_mut() {
        let entity_index = virtual_scroll_state
            .sorted_entity_ids
            .iter()
            .position(|&id| id == cached_item.entity_id);
    ...
    let entities_needing_items: Vec<u32> = virtual_scroll_state
        .sorted_entity_ids
        .iter()
        .enumerate()
        .filter(|(index, entity_id)| {
            *index >= final_start_index
                && *index < final_end_index
                && !visible_entity_ids.contains(entity_id)
        })
        ...
        commands.entity(virtual_content).with_children(|parent| {
            for entity_id in entities_needing_items {
                if let Some(entity) = virtual_scroll_state.loaded_entities.get(&entity_id) {
                    let index = virtual_scroll_state
                        .sorted_entity_ids
                        .iter()
                    ...
                    let item_entity = super::entity_list::spawn_entity_list_item(
                        &mut parent.commands(),
                        virtual_content,
                        entity,
                    );
...
pub struct CustomScrollPosition {
    ...
    pub y: f32,
    /// Maximum allowed scroll position based on content height
    pub max_y: f32,
    /// Timestamp of the last scroll event for debouncing
    pub last_scroll_time: f64,
    /// Minimum interval between scroll updates in seconds
...
pub fn handle_infinite_scroll_input(
    ...
    mut component_scroll_query: Query<
        &mut ScrollPosition,
        With<super::component_viewer::ComponentViewerPanel>,
    >,
) {
    // Update max scroll based on current content AND current container height
    let new_max_y = (virtual_scroll_state.total_content_height
            ...
            if let Some(cursor_pos) = window.cursor_position() {
                ...
                if cursor_pos.x < window_width * 0.3 {
                    ...
                    if abs_delta > virtual_scroll_state.max_scroll_velocity {
                        scroll_delta =
                            scroll_delta.signum() * virtual_scroll_state.max_scroll_velocity;
                    }
...
pub fn update_scrollbar_indicator(
    ...
    container_query: Query<
        ...
        (
            ...
            Without<ScrollbarIndicator>,
        ),
    >,
    indicator_query: Query<
        &Node,
        (
            With<ScrollbarIndicator>,
            Without<EntityListContainer>,
            Without<ScrollbarThumb>,
        ),
    >,
    ...
    let Ok(container_node) = container_query.single() else {
        return;
    };
    let Ok(indicator_node) = indicator_query.single() else {
        return;
    };
    ...
    let thumb_height = (indicator_height * content_ratio)
        .max(20.0)
