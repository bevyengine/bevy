...
use bevy_math::{Grid, GridError, IRect, IVec2, UVec2};
...
impl TileStorage {
    ...
    pub fn iter_sub_rect(
        ...
        match &self.data {
            ...
            TileStorageData::Dense(tiles) => {
                let (rows, columns) = tiles.size();
                if min.x < 0 || min.y < 0 || max.x >= columns as i32 || max.y >= rows as i32 {
                    return Err(TileStorageError::OutOfBounds { x: min.x, y: min.y });
                }
                Ok(tiles
                    .sub_rect(rect.as_urect())
                    .map(|iter| {
                        iter.map(|tile| tile.as_ref())
                            // TODO: How can we avoid this?
                            .collect::<Vec<_>>()
                            .into_iter()
                    })
                    .map_err(|err| match err {
                        GridError::IndicesOutOfBounds(x, y) => TileStorageError::OutOfBounds {
                            x: x as i32,
                            y: y as i32,
                        },
                        _ => TileStorageError::GridError(err),
                    })?)
            }
        }
    }
