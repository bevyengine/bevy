...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Calculate tile coordinate with inverted Y
    let tile_uv = vec2<f32>(in.uv.x, 1.0 - in.uv.y) * vec2<f32>(chunk_size);
    ...
    // Use original UV for texture sampling (no inversion)
    let local_uv = fract(in.uv * vec2<f32>(chunk_size));
