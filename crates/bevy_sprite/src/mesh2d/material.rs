...
impl<M: Material2d> FromWorld for Material2dPipeline<M> {
    fn from_world(world: &mut World) -> Self {
        ...
        let material2d_layout = M::bind_group_layout(device);
...
fn prepare_materials_2d<M: Material2d>(
    ...
    for (handle, material) in queued_assets {
        match prepare_material2d(&material, &device, &images, &fallback_image, &pipeline) {
            Ok(prepared_asset) => {
...
fn prepare_material2d<M: Material2d>(
    ...
    let prepared =
        material.as_bind_group(&pipeline.material2d_layout, device, images, fallback_image)?;
