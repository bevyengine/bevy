...
layout(set = 1, binding = 1) uniform ColorMaterial_border {
    float border_radius;
};

# ifdef COLORMATERIAL_TEXTURE 
layout(set = 1, binding = 2) uniform texture2D ColorMaterial_texture;
layout(set = 1, binding = 3) uniform sampler ColorMaterial_texture_sampler;
# endif
...
# endif
    ...
    if (border_radius > 0.0) {
        float d = distance_round_border((v_Uv - vec2(0.5)) * size, size * 0.5, border_radius);
        float softness = 0.33;
