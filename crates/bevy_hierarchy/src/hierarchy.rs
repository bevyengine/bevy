...
pub fn try_despawn_with_children_recursive(world: &mut World, entity: Entity, warn: bool) {
    // first, make the entity's own parent forget about it
    if let Some(parent) = world.get::<Parent>(entity).map(|parent| parent.0) {
        if let Some(mut children) = world.get_mut::<Children>(parent) {
            children.0.retain(|c| *c != entity);
...
pub trait DespawnRecursiveExt {
    ...
    fn despawn_recursive(self, warn: bool);

    /// Despawns all descendants of the given entity.
    fn despawn_descendants(&mut self, warn: bool) -> &mut Self;
}
...
impl DespawnRecursiveExt for EntityCommands<'_> {
    ...
    fn despawn_recursive(mut self, warn: bool) {
        let entity = self.id();
        self.commands().queue(DespawnRecursive { entity, warn });
    }

    fn despawn_descendants(&mut self, warn: bool) -> &mut Self {
        let entity = self.id();
        self.commands()
            .queue(DespawnChildrenRecursive { entity, warn });
        self
    }
}
...
impl<'w> DespawnRecursiveExt for EntityWorldMut<'w> {
    ...
    fn despawn_recursive(self, warn: bool) {
        ...
        despawn_with_children_recursive(self.into_world_mut(), entity, warn);
    }
...
mod tests {
    ...
    fn despawn_recursive() {
        ...
        {
            let mut commands = Commands::new(&mut queue, &world);
            commands.entity(parent_entity).despawn_recursive(true);
