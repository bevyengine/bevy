...
/// A type similar to [`UnsafeCell`] except it can be constructed from a shared or mutable reference.
///
/// This type tries to act "borrow-like" like which means that:
/// - It must always reference a valid instance of `T`.
/// - The lifetime `'a` accurately represents how long it is valid.
///
/// However, while this cell can produce a mutable reference (only if created from one),
/// it *can* be cloned, so the user is responsible for upholding Rust's aliasing rules.
/// - A `&T` reference may be released to safe code and may co-exist with other `&T` references,
/// but not with a `&mut T` reference.
/// - A `&mut T` reference may be released to safe code provided no other `&mut T` or `&T` exists. A `&mut T` must always be unique.
///
#[doc(hidden)]
pub enum SemiSafeCell<'a, T> {
    /// Cannot produce references.
    Empty,
    /// Can produce shared references.
    Ref(&'a T),
    /// Can produce one shared or mutable reference before converting to
    /// [`Ref`](SemiSafeCell::Ref) or [`Empty`](SemiSafeCell::Empty), respectively.
    Mut(&'a UnsafeCell<T>),
}
...
impl<'a, T> SemiSafeCell<'a, T> {
    ...
    ///
    /// Note: This converts a [`Mut`](SemiSafeCell::Mut) cell to a [`Ref`](SemiSafeCell::Ref) cell afterwards.
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn take_ref(&mut self) -> &'a T {
        match self {
            Self::Empty => panic!("cell is empty"),
            Self::Ref(borrow) => borrow,
            Self::Mut(cell) => {
                let borrow = &*cell.get();
                *self = SemiSafeCell::Ref(borrow);
                borrow
            }
        }
    }
    ...
    ///
    /// Note: This converts a [`Mut`](SemiSafeCell::Mut) cell to an [`Empty`](SemiSafeCell::Empty) cell afterwards.
    ///
    /// # Safety
    ///
    ...
    /// # Panics
    ///
    /// Panics if the cell is empty or was constructed from a shared reference.
    pub unsafe fn take_mut(&mut self) -> &'a mut T {
        match self {
            Self::Empty => panic!("cell is empty"),
            Self::Ref(_) => panic!("cannot return mutable reference from Ref cell"),
            Self::Mut(cell) => {
                let borrow = &mut *cell.get();
                *self = SemiSafeCell::Empty;
                borrow
            }
        }
    }
...
// SAFETY: Executor guarantees that systems with conflicting access do not run at the same time.
unsafe impl<T: Sync> Send for SemiSafeCell<'_, T> {}
