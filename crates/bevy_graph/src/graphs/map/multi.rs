...
impl<N, E, const DIRECTED: bool> Graph<N, E> for MultiMapGraph<N, E, DIRECTED> {
    ...
    fn contains_edge_between(&self, src: NodeIdx, dst: NodeIdx) -> bool {
        match self.adjacencies.get(src).unwrap().outgoing().get(&dst) {
    ...
    type EdgesOf<'e> = iters::EdgesByIdx<'e, E, &'e EdgeIdx, IterChoice<&'e EdgeIdx, std::iter::Flatten<iters::TupleExtract<&'e NodeIdx, &'e Vec<EdgeIdx>, std::iter::Chain<hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>, iters::SkipIndexIter<'e, &'e Vec<EdgeIdx>, hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>>>, false>>, iters::LoopSafetyIter<'e, E, &'e EdgeIdx, std::iter::Flatten<iters::TupleExtract<&'e NodeIdx, &'e Vec<EdgeIdx>, hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>, false>>>>> where Self: 'e;
    fn edges_of(&self, index: NodeIdx) -> Self::EdgesOf<'_> {
        let inner = if DIRECTED {
            IterChoice::new_first(
                iters::TupleExtract::new_second(self.adjacencies[index].incoming().iter().chain(
                    iters::SkipIndexIter::new(self.adjacencies[index].outgoing().iter(), index), // the outgoing will skip self to ensure loops will only come once (from incoming)
                ))
                .flatten(),
            )
        } else {
    ...
    type EdgesOfMut<'e> = iters::EdgesByIdxMut<'e, E, &'e EdgeIdx, IterChoice<&'e EdgeIdx, std::iter::Flatten<iters::TupleExtract<&'e NodeIdx, &'e Vec<EdgeIdx>, std::iter::Chain<hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>, iters::SkipIndexIter<'e, &'e Vec<EdgeIdx>, hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>>>, false>>, iters::LoopSafetyIter<'e, E, &'e EdgeIdx, std::iter::Flatten<iters::TupleExtract<&'e NodeIdx, &'e Vec<EdgeIdx>, hashbrown::hash_map::Iter<'e, NodeIdx, Vec<EdgeIdx>>, false>>>>> where Self: 'e;
    ...
    type Neighbors<'n> = iters::NodesByIdx<'n, N, &'n NodeIdx, iters::NodeJustOnceIter<&'n NodeIdx, iters::TupleExtract<&'n NodeIdx, &'n Vec<EdgeIdx>, IterChoice<(&'n NodeIdx, &'n Vec<EdgeIdx>), std::iter::Chain<hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>, hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>>, hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>>, true>>> where Self: 'n;
    ...
    type NeighborsMut<'n> = iters::NodesByIdxMut<'n, N, &'n NodeIdx, iters::NodeJustOnceIter<&'n NodeIdx, iters::TupleExtract<&'n NodeIdx, &'n Vec<EdgeIdx>, IterChoice<(&'n NodeIdx, &'n Vec<EdgeIdx>), std::iter::Chain<hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>, hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>>, hashbrown::hash_map::Iter<'n, NodeIdx, Vec<EdgeIdx>>>, true>>> where Self: 'n;
