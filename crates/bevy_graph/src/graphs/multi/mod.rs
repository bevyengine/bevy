...
mod test {
    ...
    #[macro_export]
    macro_rules! multi_graph_tests {
        ($($graph:ident )::+) => {
            use $crate::graphs::{Graph, multi::test::{self, Person}};
    ...
    pub fn undirected_edges(mut graph: impl Graph<Person, i32>) {
        ...
        let j_itself = graph.new_edge(jennifer, jennifer, 8).unwrap();
        ...
        assert_eq!(
            graph.edges_between(jennifer, jake).unwrap(),
            graph.edges_between(jake, jennifer).unwrap()
        );

        assert!(graph.edges_of(jennifer).contains(&(jennifer, j_itself)));

        assert!(unordered_eq(
            &graph.edges_between(jake, michael).unwrap(),
            &[jm, jm2]
        ));
    ...
    pub fn directed_edges(mut graph: impl Graph<Person, i32>) {
        ...
        assert!(unordered_eq(
            &graph.edges_of(jake),
            &[(michael, jm), (michael, jm2), (other_jake, jo)]
        ));
        ...
        assert!(!graph.edges_between(jennifer, jake).unwrap().is_empty());
        assert!(graph.edges_between(jake, jennifer).unwrap().is_empty());
    ...
    pub fn remove_node_undirected(mut graph: impl Graph<Person, i32>) {
        ...
        assert_eq!(
            graph.edges_between(jake, michael).unwrap()[0]
                .get(&graph)
                .unwrap(),
            &20
        );
        assert_eq!(
            graph.edges_between(michael, jake).unwrap()[0]
                .get(&graph)
                .unwrap(),
            &20
        );
