...
use bevy_text::{FontFamily, TextFont};
...
pub struct InheritableFont {
    ...
    pub font: HandleOrPath<FontFamily>,
...
impl InheritableFont {
    ...
    pub fn from_handle(handle: Handle<FontFamily>) -> Self {
        Self {
...
pub(crate) fn on_changed_font(
    ...
    if let Ok(style) = font_style.get(insert.entity)
        && let Some(font) = match style.font {
            ...
            HandleOrPath::Path(ref p) => Some(assets.load::<FontFamily>(p)),
        }
    {
