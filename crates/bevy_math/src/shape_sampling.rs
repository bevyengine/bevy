...
pub trait ShapeSample {
    ...
    /// ```
    fn sample_volume(&self, rng: &mut impl Rng) -> Self::Output;
    ...
    /// ```
    fn sample_surface(&self, rng: &mut impl Rng) -> Self::Output;
}

impl ShapeSample for Circle {
    type Output = Vec2;

    fn sample_volume(&self, rng: &mut impl Rng) -> Vec2 {
    ...
    fn sample_surface(&self, rng: &mut impl Rng) -> Vec2 {
        let theta = rng.gen_range(0.0..TAU);
