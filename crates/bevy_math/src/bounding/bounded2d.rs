...
fn test_aabb2d_center() {
    ...
    assert!((aabb.center() - Vec2::new(0.25, 0.)).length() < 0.0001);
    let aabb = Aabb2d {
        min: Vec2::new(5., -10.),
        max: Vec2::new(10., -5.),
    };
    assert!((aabb.center() - Vec2::new(7.5, -7.5)).length() < 0.0001);
}
...
fn test_aabb2d_area() {
    ...
    assert!((aabb.visible_area() - 4.).abs() < 0.0001);
    let aabb = Aabb2d {
        min: Vec2::new(0., 0.),
        max: Vec2::new(1., 0.5),
    };
    assert!((aabb.visible_area() - 0.5).abs() < 0.0001);
}

#[test]
fn test_aabb2d_contains() {
    let a = Aabb2d {
        min: Vec2::new(-1., -1.),
        max: Vec2::new(1., 1.),
    };
...
fn test_aabb2d_merged() {
    ...
    assert!((merged.min - Vec2::new(-2., -1.)).length() < 0.0001);
    assert!((merged.max - Vec2::new(1., 1.)).length() < 0.0001);
}

#[test]
fn test_aabb2d_padded() {
    let a = Aabb2d {
        min: Vec2::new(-1., -1.),
        max: Vec2::new(1., 1.),
    };
    let padded = a.padded((Vec2::ONE, Vec2::Y));
    assert!((padded.min - Vec2::new(-2., -2.)).length() < 0.0001);
    assert!((padded.max - Vec2::new(1., 2.)).length() < 0.0001);
}

#[test]
fn test_aabb2d_shrunk() {
    let a = Aabb2d {
        min: Vec2::new(-1., -1.),
        max: Vec2::new(1., 1.),
    };
    let shrunk = a.shrunk((Vec2::ONE, Vec2::Y));
    assert!((shrunk.min - Vec2::new(-0., -0.)).length() < 0.0001);
    assert!((shrunk.max - Vec2::new(1., 0.)).length() < 0.0001);
}
...
#[test]
fn test_bounding_circle_contains() {
...
#[test]
fn test_bounding_circle_merged() {
    let a = BoundingCircle {
        center: Vec2::ONE,
        radius: 5.,
    };
    ...
    let merged = a.merged(&b);
    assert!((merged.center - Vec2::new(1., 0.5)).length() < 0.0001);
    assert!((merged.radius - 5.5).abs() < 0.0001);
}

#[test]
fn test_bounding_circle_padded() {
    let a = BoundingCircle {
        center: Vec2::ONE,
        radius: 5.,
    };
...
#[test]
fn test_bounding_circle_shrunk() {
    let a = BoundingCircle {
        center: Vec2::ONE,
        radius: 5.,
    };
