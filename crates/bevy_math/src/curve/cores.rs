...
//!
//! The `Core` types here expose their fields publically so that it is easier to extend them,
//! but in doing so, you must maintain the invariants of those fields yourself. The provided
//! methods all maintain the invariants, so this only concerns you if you manually mutate them.
...
/// Given a domain and a number of samples taken over that interval, return a [`InterpolationDatum`]
