...
mod tests {
    ...
    fn local_ray_cast_segment() {
        ...
        assert!(!segment.intersects_local_ray(Ray2d::new(Vec2::new(2.0, 0.25), Dir2::NEG_X)));
        assert!(!segment.intersects_local_ray(Ray2d::new(Vec2::new(0.0, 0.5), Dir2::NEG_Y)));
        assert!(!segment.intersects_local_ray(Ray2d::new(Vec2::new(0.0, 1.1), Dir2::Y)));

        // Ray hits the circular segment.
        assert!(segment.intersects_local_ray(Ray2d::new(Vec2::new(2.0, 0.75), Dir2::NEG_X)));
        assert!(segment.intersects_local_ray(Ray2d::new(Vec2::new(0.0, 0.9), Dir2::Y)));
        assert!(segment.intersects_local_ray(Ray2d::new(Vec2::new(0.0, 1.1), Dir2::NEG_Y)));
        ...
        let ray = Ray2d::new(Vec2::new(0.0, segment.apothem()), Dir2::Y);
        let hit = segment.local_ray_cast(ray, f32::MAX, true);
        assert_eq!(hit, Some(RayHit2d::new(0.0, Dir2::NEG_Y)));

        // Interior hit for hollow segment.
        let ray = Ray2d::new(Vec2::new(0.0, segment.apothem() + 0.01), Dir2::Y);
