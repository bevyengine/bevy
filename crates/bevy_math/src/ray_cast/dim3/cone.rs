        ...
        if is_inside {
            ...
            if discriminant < 0.0 {
                return if ray.direction.y == -1.0 {
                    ...
                    Some(RayHit3d::new(ops::sqrt(top_distance_squared), Dir3::Y))
                } else {
                    None
                };
            }
...
mod tests {
    ...
    fn local_ray_cast_cone() {
        ...
        // Ray hits the cone.
        assert!(cone.intersects_local_ray(Ray3d::new(Vec3::new(0.0, 0.9, 0.0), Dir3::Y)));
        assert!(cone.intersects_local_ray(Ray3d::new(Vec3::new(0.4, 0.0, 0.0), Dir3::X)));

        // Ray points away from the cone.
        assert!(!cone.intersects_local_ray(Ray3d::new(Vec3::new(0.0, 1.1, 0.0), Dir3::Y)));
        assert!(!cone.intersects_local_ray(Ray3d::new(Vec3::new(0.6, 0.0, 0.0), Dir3::X)));

        // Edge case: The ray is pointing straight down at the tip.
        let ray = Ray3d::new(Vec3::new(0.0, 1.1, 0.0), Dir3::NEG_Y);
        let hit = cone.local_ray_cast(ray, f32::MAX, true).unwrap();
