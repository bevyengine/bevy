...
fn torus_ray_distance(
    ...
    if ops::abs(k3 * (k3 * k3 - k2) + k1) < 0.01 {
        po = -1.0;
    ...
    if h < 0.0 {
        // 4 intersections
        let q_sqrt = ops::sqrt(q);
        z = 2.0 * q_sqrt * ops::cos(ops::acos(r / (q * q_sqrt)) / 3.0);
    } else {
        // 2 intersections
        let q_sqrt = ops::cbrt(ops::sqrt(h) + ops::abs(r));
        z = r.signum() * ops::abs(q_sqrt + q / q_sqrt);
    }
    ...
    if discriminant1 > 0.0 {
        let d_sqrt = ops::sqrt(discriminant1);
        let (t1, t2) = if po < 0.0 {
    ...
    if discriminant2 > 0.0 {
        let d_sqrt = ops::sqrt(discriminant2);
        let (t1, t2) = if po < 0.0 {
            (2.0 / (d1 - d_sqrt - k3), 2.0 / (d1 + d_sqrt - k3))
        } else {
            (d1 - d_sqrt - k3, d1 + d_sqrt - k3)
...
mod tests {
    ...
    fn local_ray_cast_torus() {
        ...
        let ray = Ray3d::new(Vec3::new(0.75, 0.0, 0.0), Dir3::X);
        let hit = torus.local_ray_cast(ray, f32::MAX, true).unwrap();
        ...
        let ray = Ray3d::new(Vec3::new(2.0, 0.0, 0.0), Dir3::Y);
        assert!(!torus.intersects_local_ray(ray));

        // Hit distance exceeds max distance.
        let ray = Ray3d::new(Vec3::new(2.0, 0.0, 0.0), Dir3::NEG_Y);
