...
impl PrimitiveRayCast3d for Capsule3d {
    ...
    fn local_ray_cast(&self, ray: Ray3d, max_distance: f32, solid: bool) -> Option<RayHit3d> {
        ...
        if d >= 0.0 {
            ...
            let is_inside_cylinder_vertical = ops::abs(ray.origin.y) < self.half_length;
...
mod tests {
    ...
    fn local_ray_cast_capsule_3d() {
        ...
        let ray = Ray3d::new(Vec3::new(-2.0, 1.0 + circle_frac_pi_4_y, 0.0), Dir3::X);
        ...
        let ray = Ray3d::new(Vec3::new(0.0, -1.0, 0.0), Dir3::Y);
        let hit = capsule.local_ray_cast(ray, f32::MAX, false);
        assert_eq!(hit, Some(RayHit3d::new(3.0, Dir3::NEG_Y)));

        // Ray points away from the capsule.
        assert!(!capsule.intersects_local_ray(Ray3d::new(Vec3::new(0.0, 2.1, 0.0), Dir3::Y)));

        // Hit distance exceeds max distance.
        let ray = Ray3d::new(Vec3::new(0.0, 2.6, 0.0), Dir3::NEG_Y);
