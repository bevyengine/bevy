...
impl PrimitiveRayCast3d for ConicalFrustum {
    #[inline]
    fn local_ray_cast(&self, ray: Ray3d, max_distance: f32, solid: bool) -> Option<RayHit3d> {
        ...
        let radius_bottom_squared = self.radius_bottom * self.radius_bottom;
        let radius_top_squared = self.radius_top * self.radius_top;
        ...
        // The ray origin is inside of the frustum if both of the following are true:
        ...
        let is_inside = oa_dot_ba >= 0.0 && oa_dot_ba <= height_squared && {
            ...
            // let slope = self.height / (self.radius_bottom - self.radius_top);
            //
            // let delta_radius = y / slope
            //                  = y / (self.height / (self.radius_bottom - self.radius_top))
            //                  = y * (self.radius_bottom - self.radius_top) / self.height
            //                  = ob_dot_ba / self.height * (self.radius_bottom - self.radius_top) / self.height
            //                  = ob_dot_ba * (self.radius_bottom - self.radius_top) / (self.height * self.height);
            // let radius = self.radius_bottom + delta_radius;
            let delta_radius = ob_dot_ba * (self.radius_bottom - self.radius_top) / height_squared;
            let radius = self.radius_bottom + delta_radius;
        ...
        if is_inside {
            ...
            if oa_dot_ba >= 0.0 && dir_dot_ba > 0.0 {
                ...
                if (oa * dir_dot_ba - ray.direction * oa_dot_ba).length_squared()
                    < radius_top_squared * dir_dot_ba * dir_dot_ba
                {
            ...
            // The ray hit the lateral surface of the conical frustum.
            // Because the ray is known to be inside of the shape, no further checks are needed.

            let radius_difference = self.radius_top - self.radius_bottom;
            let height_pow_4 = height_squared * height_squared;
            let hypot_squared = height_squared + radius_difference * radius_difference;
            ...
            let b = height_pow_4 * dir_dot_oa - oa_dot_ba * dir_dot_ba * hypot_squared
                + height_squared * self.radius_top * radius_difference * dir_dot_ba;
            let c = height_pow_4 * top_distance_squared - oa_dot_ba * oa_dot_ba * hypot_squared
                + height_squared
                    * self.radius_top
                    * (radius_difference * oa_dot_ba * 2.0 - height_squared * self.radius_top);
            ...
            let distance = (-b + ops::sqrt(discriminant)) / a;

            if distance < 0.0 || distance > max_distance {
                return None;
            }

            // Squared distance from top along frustum axis at the point of intersection
            let hit_y_squared = oa_dot_ba + distance * dir_dot_ba;

            let normal = -Dir3::new(
                height_squared
                    * (height_squared * (oa + distance * ray.direction)
                        + radius_difference * ba * self.radius_top)
                    - ba * hypot_squared * hit_y_squared,
            )
            .ok()?;
            ...
            // Check for intersections with the lateral surface of the conical frustum.
            ...
            if hit_y_squared < 0.0 || hit_y_squared > height_squared {
                // The point of intersection is outside of the height of the frustum.
                return None;
            }
...
mod tests {
    ...
    #[test]
    fn local_ray_cast_conical_frustum() {
        ...
        let ray = Ray3d::new(Vec3::new(2.0, 0.0, 0.0), Dir3::NEG_X);
        let hit = frustum.local_ray_cast(ray, f32::MAX, true).unwrap();
        assert_eq!(hit.distance, 1.25);
        assert_relative_eq!(hit.normal, Dir3::from_xyz(4.0, 1.0, 0.0).unwrap());

        // Ray origin is inside of the solid frustum.
        let ray = Ray3d::new(Vec3::ZERO, Dir3::X);
        let hit = frustum.local_ray_cast(ray, f32::MAX, true);
        assert_eq!(hit, Some(RayHit3d::new(0.0, Dir3::NEG_X)));

        // Ray origin is inside of the hollow frustum.
        let ray = Ray3d::new(Vec3::ZERO, Dir3::X);
        let hit = frustum.local_ray_cast(ray, f32::MAX, false).unwrap();
        assert_eq!(hit.distance, 0.75);
        assert_relative_eq!(hit.normal, Dir3::from_xyz(-4.0, -1.0, 0.0).unwrap());
        let ray = Ray3d::new(Vec3::ZERO, Dir3::Y);
        let hit = frustum.local_ray_cast(ray, f32::MAX, false);
        assert_eq!(hit, Some(RayHit3d::new(1.0, Dir3::NEG_Y)));

        // Ray hits the frustum.
        assert!(frustum.intersects_local_ray(Ray3d::new(Vec3::new(0.0, 0.9, 0.0), Dir3::Y)));
        assert!(frustum.intersects_local_ray(Ray3d::new(Vec3::new(0.5, 0.5, 0.0), Dir3::X)));

        // Ray points away from the frustum.
        assert!(!frustum.intersects_local_ray(Ray3d::new(Vec3::new(0.0, 1.1, 0.0), Dir3::Y)));
        assert!(!frustum.intersects_local_ray(Ray3d::new(Vec3::new(0.75, 0.5, 0.0), Dir3::X)));

        // Hit distance exceeds max distance.
        let ray = Ray3d::new(Vec3::new(0.0, 2.0, 0.0), Dir3::NEG_Y);
        let hit = frustum.local_ray_cast(ray, 0.5, true);
        assert!(hit.is_none());
    }
}
