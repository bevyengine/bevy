...
/// type `Scalar` is used to interpolate between `self` and `rhs`
/// it should provide a way to range from `self` to `rhs` linearily
/// and be able to extrapolate if the user input a value less than min
/// or greater than max.
pub trait Lerp {
    ...
    ///
    /// If `Scalar` is f32 or f64, when `s` is `0.0`, the result will be equal to `self`.
    /// When `s` is `1.0`, the result will be equal to `rhs`. When `s` is outside of range `[0, 1]`,
