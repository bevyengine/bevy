use crate::{
    ...
    render_resource::{
        BindGroupLayout, OwnedRenderPipelineDescriptor, ProcessShaderError, RenderPipeline, Shader,
        ShaderProcessor,
    },
...
use thiserror::Error;
use wgpu::{
    FragmentState, PipelineLayoutDescriptor, RenderPipelineDescriptor, ShaderModule,
...
impl ShaderCache {
    fn get(
        ...
        // PERF: this shader_defs clone isn't great. try Cow?
        let module = data
            .processed_shaders
            .entry(shader_defs.to_vec())
            .or_insert_with(|| {
                // TODO: handle process errors properly
                let processed = self.processor.process_shader(shader, &shader_defs).unwrap();
...
#[derive(Default)]
struct LayoutCache {
    layouts: HashMap<PipelineLayoutKey, wgpu::PipelineLayout>,
}
...
struct CachedPipeline {
    descriptor: OwnedRenderPipelineDescriptor,
...
impl RenderPipelineCache {
    ...
    pub fn queue(&mut self, descriptor: OwnedRenderPipelineDescriptor) -> CachedPipelineId {
    ...
    pub fn process_queue(&mut self, shaders: &RenderAssets<Shader>) {
        ...
        for id in pipelines {
            ...
            let descriptor = RenderPipelineDescriptor {
                ...
                label: descriptor.label.as_ref().map(|label| label.deref()),
                layout,
