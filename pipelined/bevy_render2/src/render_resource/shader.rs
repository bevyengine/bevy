...
impl Default for ShaderProcessor {
    fn default() -> Self {
        Self {
            ifdef_regex: Regex::new(r"#(\s*)?ifdef(\s*)([\w|\d|_]+)").unwrap(),
            ifndef_regex: Regex::new(r"#(\s*)?ifndef(\s*)([\w|\d|_]+)").unwrap(),
            endif_regex: Regex::new(r"#(\s)?endif").unwrap(),
        }
    }
}
