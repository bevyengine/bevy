...
/// [`Cube`](crate::mesh::shape::Cube) or [`Box`](crate::mesh::shape::Box), but you can also construct
/// one yourself.
///
/// For a single mesh, all of the vertex attributes must have the same length.
///
...
pub struct Mesh {
    ...
    /// [`BTreeMap`](std::collections::BTreeMap) with all defined vertex attributes (Positions, Normals, ...)
...
/// Calculates the face normal of the triangle described by the three points.
fn face_normal(a: [f32; 3], b: [f32; 3], c: [f32; 3]) -> [f32; 3] {
    let (a, b, c) = (Vec3::from(a), Vec3::from(b), Vec3::from(c));
    (b - a).cross(c - a).normalize().into()
}

/// Extends the [`VertexFormat`] with the `get_size` methode.
