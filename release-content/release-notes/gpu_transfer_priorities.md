...
The `RenderAssetBytesPerFrame` resource now supports a `MaxBytesWithPriority` mode. Individual assets can be assigned a priority of `Immediate` or a `Priority(i16)` priority level. This means essential resources can be guaranteed to update the same frame, and you can prioritize other assets as you choose.
...
For a demo of the behavior, see the new `gpu_transfer_limits` example.
