...
In previous versions of Bevy, render [`Plugin`] code had to look different than normal code, due to how the renderer was initialized. In general, renderer resources and systems had to be added in `Plugin::finish`, separate from the "normal" spot: `Plugin::build`. The fact that `Plugin::build` resulted in the "correct" order was a bit arbitrary / incidental.
