...
- It is now the othe way around: Denying `A`, which is required *by* `C`, will now also deny `C`. This can be bypassed with the new `without_required_by_components` method.
