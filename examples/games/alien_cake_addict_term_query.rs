...
fn teardown(
    ...
    entities: TermQuery<Entity, (Without<Camera>, Without<Window>)>,
) {
...
fn move_player(
    ...
    mut transforms: TermQuery<&mut Transform>,
    time: Res<Time>,
) {
...
fn focus_camera(
    ...
    mut transforms: ParamSet<(
        TermQuery<&mut Transform, With<Camera3d>>,
        TermQuery<&Transform>,
    )>,
) {
    const SPEED: f32 = 2.0;
...
fn rotate_bonus(game: Res<Game>, time: Res<Time>, mut transforms: TermQuery<&mut Transform>) {
    if let Some(entity) = game.bonus.entity {
...
fn scoreboard_system(game: Res<Game>, mut query: TermQuery<&mut Text>) {
