...
struct State {
    // vector of schedule/nodeid -> text entity
    systems: Vec<(InternedScheduleLabel, NodeId, Entity)>,
        ...
        .with_children(|parent| {
            ...
            for label in schedule_order {
                ...
                for (node_id, system) in systems {
                    ...
                    state.systems.push((*label, node_id, id));
...
fn update_ui(
    ...
    for (schedule, system, text_entity) in &state.systems {
        let mark = if &cursor_schedule == schedule && *system == cursor_system {
            "-> "
        } else {
            "   "
        };
        let mut text = sections.get_mut(*text_entity).unwrap();
        text.value = mark.to_string();
    }
}
