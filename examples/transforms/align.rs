...
fn draw_ship_axes(mut gizmos: Gizmos, query: Query<&Transform, With<Ship>>) {
    let ship_transform = query.get_single().unwrap();
...
fn draw_random_axes(mut gizmos: Gizmos, query: Query<&RandomAxes>) {
    let RandomAxes(v1, v2) = query.get_single().unwrap();
...
fn rotate_ship(mut ship: Query<(&mut Ship, &mut Transform)>) {
    let (mut ship, mut ship_transform) = ship.get_single_mut().unwrap();
...
fn handle_keypress(
    ...
    let (mut ship, ship_transform) = ship.get_single_mut().unwrap();
    let mut random_axes = random_axes.get_single_mut().unwrap();
    ...
    if keyboard.just_pressed(KeyCode::KeyH) {
        let mut instructions_viz = instructions.get_single_mut().unwrap();
