...
fn main() {
    ...
    // Pretend it's just some `Box<dyn Reflect + Send + Sync>` value.
    let mut value: Box<dyn Reflect + Send + Sync> = Box::new(Zombie { health: 100 });
    ...
    struct ReflectDamageable {
        ...
        damage: fn(&mut (dyn Reflect + Send + Sync), damage: Box<dyn Reflect + Send + Sync>),
    }
    ...
    impl<
            T: Reflect
                + Send
                + Sync
                + Damageable<Health: Reflect + Send + Sync + Send + Sync>
                + Send
                + Sync,
        > FromType<T> for ReflectDamageable
    {
        fn from_type() -> Self {
            Self {
    ...
    let value: Box<dyn Reflect + Send + Sync> = Box::new(Skeleton { health: 50 });
    ...
    // which convert `&(dyn Reflect + Send + Sync)` into `&dyn MyTrait`, `&mut (dyn Reflect + Send + Sync)` into `&mut dyn MyTrait`,
    // and `Box<dyn Reflect + Send + Sync>` into `Box<dyn MyTrait>`, respectively.
