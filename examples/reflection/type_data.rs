...
fn main() {
    ...
    // Now, we can create a blanket implementation of the `FromType` trait to construct our type
    // data for any type that implements `Reflect` and `Damageable`.
    impl<T: Reflect + Damageable<Health: Reflect>> FromType<T> for ReflectDamageable {
        fn from_type() -> Self {
            Self {
                damage: |reflect, damage| {
                    // This requires that `reflect` is `T` and not a dynamic representation like
                    // `DynamicStruct`. We could have the function pointer
                    // return a `Result`, but we'll just `unwrap` for simplicity.
    ...
    // the derive macro will automatically look for a type named `ReflectDamageable` in the current
    // scope.
    #[reflect(Damageable)]
    struct Skeleton {
        health: u32,
    ...
    // Bevy provides a convenience macro for generating type data that converts `dyn Reflect` into
    // `dyn MyTrait`.
    #[reflect_trait]
    trait Health {
        fn health(&self) -> u32;
    ...
    // Using the `#[reflect_trait]` macro we're able to automatically generate a `ReflectHealth`
    // type data struct, which can then be registered like any other type data:
    ...
    // Type data generated by `#[reflect_trait]` comes with a `get`, `get_mut`, and `get_boxed`
    // method, which convert `&dyn Reflect` into `&dyn MyTrait`, `&mut dyn Reflect` into `&mut
    // dyn MyTrait`, and `Box<dyn Reflect>` into `Box<dyn MyTrait>`, respectively.
    let value: &dyn Health = reflect_health.get(value.as_reflect()).unwrap();
    assert_eq!(value.health(), 50);

    // Lastly, here's a list of some useful type data provided by Bevy that you might want to
    // register for your types:
