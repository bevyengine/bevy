...
fn update_ui_state(
    ...
    if !currently_selected_option.is_changed() && !cameras.get_single().unwrap().is_changed() {
        return;
    }
    ...
    help_text.get_single_mut().unwrap().sections[0].value =
        create_help_text(&currently_selected_option);
}
...
fn adjust_color_grading_option(
    ...
    if delta != 0.0 {
        let mut color_grading = cameras.get_single_mut().unwrap();
