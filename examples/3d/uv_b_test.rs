...
fn bw_checkerboard_texture() -> Image {
    const TEXTURE_SIZE: usize = 8;
    const BLACK: [u8; 4] = [0, 0, 0, 255]; // RGBA for black
    const WHITE: [u8; 4] = [255, 255, 255, 255]; // RGBA for white
    ...
    for y in 0..TEXTURE_SIZE {
        for x in 0..TEXTURE_SIZE {
            ...
            if (x + y) % 2 == 0 {
                texture_data[offset..offset + 4].copy_from_slice(&WHITE);
            } else {
                texture_data[offset..offset + 4].copy_from_slice(&BLACK);
            }
        }
    }
