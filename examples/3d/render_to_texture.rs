...
fn rotator_system(time: Res<Time>, mut query: Query<&mut Transform, With<FirstPassCube>>) {
    for mut transform in &mut query {
        transform.rotate_x(Angle::radians(1.5 * time.delta_seconds()));
        transform.rotate_z(Angle::radians(1.3 * time.delta_seconds()));
    }
}
...
fn cube_rotator_system(time: Res<Time>, mut query: Query<&mut Transform, With<MainPassCube>>) {
    for mut transform in &mut query {
        transform.rotate_x(Angle::radians(1.0 * time.delta_seconds()));
        transform.rotate_y(Angle::radians(0.7 * time.delta_seconds()));
    }
}
