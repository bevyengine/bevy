...
fn toggle_tonemapping_method(
    ...
    let mut method = tonemapping.get_single_mut().unwrap();
    let mut color_grading = color_grading.get_single_mut().unwrap();
...
fn update_color_grading_settings(
    ...
    let method = tonemapping.get_single().unwrap();
...
fn update_ui(
    ...
    let old_text = &text_query.get_single().unwrap().sections[0].value;

    if *hide_ui {
        if !old_text.is_empty() {
            // get_single_mut().unwrap() always triggers change detection,
            // so only access if text actually needs changing
            text_query.get_single_mut().unwrap().sections[0]
                .value
                .clear();
        }
        return;
    ...
    let (method, color_grading) = settings.get_single().unwrap();
    let method = *method;
    ...
    if text != old_text.as_str() {
        ...
        text_query.get_single_mut().unwrap().sections[0].value = text;
    }
}
