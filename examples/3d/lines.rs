...
impl From<LineList> for Mesh {
    fn from(line: LineList) -> Self {
        let (vertices, normals): (Vec<_>, Vec<_>) = line
            .lines
            .iter()
            .flat_map(|(start, end)| {
                let first = (start.to_array(), Vec3::ZERO.to_array());
                let second = (end.to_array(), Vec3::ZERO.to_array());
                [first, second]
            })
            .unzip();
...
impl From<LineStrip> for Mesh {
    fn from(line: LineStrip) -> Self {
        let (vertices, normals): (Vec<_>, Vec<_>) = line
            .points
            .iter()
            .map(|pos| (pos.to_array(), Vec3::ZERO.to_array()))
            .unzip();
