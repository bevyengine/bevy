...
fn spawn_row(
    ...
    input.observe(
        move |on_submit: On<TextSubmission>, mut text_query: Query<&mut Text>| {
            if let Ok(mut target) = text_query.get_mut(on_submit.entity) {
                target.0 = on_submit.text.clone();
            }
        },
    );

    if is_password {
        input.insert(PasswordMask::default());
    }
