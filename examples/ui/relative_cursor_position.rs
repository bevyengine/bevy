...
fn relative_cursor_position_system(
    ...
    mut output_query: Query<&mut TextSection>,
) {
    let relative_cursor_position = relative_cursor_position_query.single();

    let mut output = output_query.single_mut();

    output.value = if let Some(relative_cursor_position) = relative_cursor_position.normalized {
        format!(
            "({:.1}, {:.1})",
    ...
    output.style.color = if relative_cursor_position.mouse_over() {
