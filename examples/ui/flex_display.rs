...
#[derive(Component)]
struct ButtonTarget{ id: Entity, color: Color }
    ...
    commands.spawn(
                ...
                parent.spawn((
                    NodeBundle {
                        style: Style {
                            ...
                            padding: UiRect { left: Val::Px(5.), top: Val::Px(5.), ..Default::default() },
                            ..Default::default()
                        },
                                ...
                                .with_children(|parent| {
                                    let id = parent.spawn((
                                        NodeBundle {
                                            ...
                                            background_color: BackgroundColor(Color::rgb(0.0, 0.0, 0.6)),
                                            ..Default::default()
                                        },
                                    )).id();
                                    target_ids.push(id);
        ...
        parent.spawn(NodeBundle {
            ...
            parent.spawn((
                ...
                ButtonTarget { id: target_ids.pop().unwrap(), color: Color::rgb(0.0, 0.0, 0.3) },
            ))
            .with_children(|parent| {
                parent.spawn(
                    TextBundle {
                ...
                parent.spawn((
                    ...
                    ButtonTarget { id: target_ids.pop().unwrap(), color: Color::rgb(0.0, 0.0, 0.4) },
                ))
                .with_children(|parent| {
                    parent.spawn(
                        TextBundle {
                    ...
                    parent.spawn((
                        ...
                        ButtonTarget { id: target_ids.pop().unwrap(), color: Color::rgb(0.0, 0.0, 0.5) },
                    ))
                    .with_children(|parent| {
                        parent.spawn(
                            TextBundle {
                        ...
                        parent.spawn((
                            ...
                            ButtonTarget { id: target_ids.pop().unwrap(), color: Color::rgb(0.0, 0.0, 0.6) },
                        ))
                        .with_children(|parent| {
                            parent.spawn(
                                TextBundle {
...
fn update(
    ...
    mut left_query: Query<(&mut BackgroundColor, &mut Style, &mut Visibility), Without<Interaction>>,
    ...
    right_query.for_each_mut(|(mut background_color, button_target, interaction)| {
        match interaction {
            Interaction::Hovered => {
                let (mut left_background_color, mut style, mut visibility) = left_query.get_mut(button_target.id).unwrap();
...
fn update_text(
    ...
    text_query.for_each_mut(|(mut text, parent)| {
        ...
        if let Ok((style, visibility)) = left_query.get(target_id) {
            text.sections[0].value = format!("Display::{:?}\nVisbility::{:?}", style.display, visibility);
        }
    });
}