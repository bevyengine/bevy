...
fn infotext_system(mut commands: Commands<'_, '_>, asset_server: Res<'_, AssetServer>) {
    ...
    let right_column = commands
        ...
        .with_children(|builder| {
            ...
            builder.spawn((Text::new("This text is left-justified and is vertically positioned to distribute the empty space equally above and below it."), TextFont { font: font.clone(), font_size: 29.0, ..default() }, TextColor(YELLOW.into()), TextLayout::new_with_justify(JustifyText::Left), Style { max_width: Val::Px(300.), ..default() }, BackgroundColor(background_color)));

            builder.spawn((Text::new("This text is fully justified and is positioned in the same way."), TextFont { font: font.clone(), font_size: 29.0, ..default() }, TextLayout::new_with_justify(JustifyText::Justified), TextColor(GREEN_YELLOW.into()), Style { max_width: Val::Px(300.), ..default() }, BackgroundColor(background_color)));
...
fn change_text_system(
    mut fps_history: Local<'_, VecDeque<f64>>,
    mut time_history: Local<'_, VecDeque<Duration>>,
    time: Res<'_, Time>,
