...
impl Default for CursorRay {
    fn default() -> Self {
        Self(Ray2d::new(Vec2::ZERO, Dir2::Y))
    }
}
...
fn setup(mut commands: Commands) {
    let shapes = [
        ...
        Shape2d::Segment(Segment2d::new(Vec2::ZERO, Vec2::new(1.0, 0.5) * 200.0)),
        Shape2d::Polyline(Polyline2d::new([
            Vec2::new(-120.0, -50.0),
            Vec2::new(-30.0, 30.0),
            Vec2::new(50.0, -40.0),
            Vec2::new(120.0, 50.0),
...
fn ray_follow_cursor(
    ...
    if let Some(cursor_world_pos) = windows
        .cursor_position()
...
impl<Config, Clear> GizmoPrimitive2d<Shape2d> for GizmoBuffer<Config, Clear>
where
    ...
    Clear: 'static + Send + Sync,
{
    ...
    fn primitive_2d(
        ...
        match &primitive {
            Shape2d::Circle(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Arc(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::CircularSector(shape) => self.primitive_2d(shape, isometry, color),
            Shape2d::CircularSegment(shape) => self.primitive_2d(shape, isometry, color),
            Shape2d::Ellipse(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Annulus(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Rectangle(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Rhombus(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Line(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Segment(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Polyline(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Polygon(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::RegularPolygon(shape) => self.primitive_2d(shape, isometry, color),
            Shape2d::Triangle(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
            Shape2d::Capsule(shape) => {
                self.primitive_2d(shape, isometry, color);
            }
        }
    }
