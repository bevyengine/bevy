...
fn gamepad_system(
    ...
    for gamepad in lobby.gamepads.iter().cloned() {
        if button_inputs.just_pressed(GamepadButton(gamepad, GamepadButtonCode::South)) {
            println!("{:?} just pressed South", gamepad);
        } else if button_inputs.just_released(GamepadButton(gamepad, GamepadButtonCode::South)) {
            println!("{:?} just released South", gamepad);
        }

        let right_trigger = button_axes
            .get(GamepadButton(gamepad, GamepadButtonCode::RightTrigger2))
            .unwrap();
        ...
        let left_stick_x = axes
            .get(GamepadAxis(gamepad, GamepadAxisCode::LeftStickX))
            .unwrap();
