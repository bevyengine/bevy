...
// The A component is extracted automatically through `ExtractComponentPlugin`.
//
// It is required to implement `ExtractComponent`. A custom implementation can be provided
// if you wish to alter the A component when extracting, rather than simply cloning it.
//
...
// The C component is the same B, but it only syncs when the `Space` key is pressed.
#[derive(Component, Clone)]
#[require(SyncToRenderWorld)]
struct C(pub f32);
...
// Resource inserted in each world, read when displaying its name.
#[derive(Resource)]
...
// System that Extracts B unconditionally, and C when the `Space` key was pressed (see the `trigger_extraction` system).
