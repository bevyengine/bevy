...
fn attack_hits(trigger: On<Attack>, name: Query<&Name>) {
    if let Ok(name) = name.get(trigger.entity().unwrap()) {
...
fn block_attack(mut trigger: On<Attack>, armor: Query<(&Armor, &Name)>) {
    let (armor, name) = armor.get(trigger.entity().unwrap()).unwrap();
...
fn take_damage(
    ...
    let (mut hp, name) = hp.get_mut(trigger.entity().unwrap()).unwrap();
