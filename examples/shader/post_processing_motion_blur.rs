...
//! Here a motion blur is applied to a 3d scene containing a rotating cube.
//! This example is shows how a storage texture can be added to the material and then used in the shader to persist
//! inter-frame data.
...
fn setup(
    ...
    storage_image.resize(size);

    let storage_image_handle = images.add(storage_image);
    ...
    let material_handle = post_processing_materials.add(PostProcessingMaterial {
        ...
        storage_texture: storage_image_handle,
    });
...
struct PostProcessingMaterial {
    ...
    #[storage_texture(2, format = "rgba8unorm")]
    storage_texture: Handle<Image>,
}

impl Material2d for PostProcessingMaterial {
    fn fragment_shader() -> ShaderRef {
        "shaders/custom_material_motion_blur.wgsl".into()
    }
}
