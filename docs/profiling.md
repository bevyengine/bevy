...
On Mac OS, Tracy can be installed through Homebrew by running `brew install tracy`, and the GUI client can be launched by running `tracy`. 
...
Note that on Windows, the capture tool is called `capture.exe`. On Mac OS, it is `tracy-capture` — e.g. `tracy-capture -o my_capture.tracy`. 
