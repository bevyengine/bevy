...
fn spawn_enemy(mut commands: Commands, keyboard_input: Res<ButtonInput<PhysicalKey>>) {
...
fn spawn_enemy_using_input_resource() {
    ...
    // Setup test resource
    let mut input = ButtonInput::<PhysicalKey>::default();
    input.press(PhysicalKey::Space);
    ...
    // Clear the `just_pressed` status for all `PhysicalKey`s
    app.world.resource_mut::<ButtonInput<PhysicalKey>>().clear();
