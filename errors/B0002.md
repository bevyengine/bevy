...
fn play_with_time(
    mut time_ticker: ResMut<Time>,
    time_reader: Res<Time>,
) {
    // ...
}
...
fn main() {
    App::new()
        ...
        .add_systems(Update, play_with_time)
        .run();
}
```

This will panic, as it's not possible to have both a mutable and an immutable resource on `Time` at the same time.

As a mutable resource already provides access to the current resource value, you can remove the immutable resource.
