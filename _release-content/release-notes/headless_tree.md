...
`SelectedTreeItem` on the root holds the selected row and the `Expanded` marker component holds the expansion state, so a component hook can keep the accessibility tree in sync. Rows that set `expandable` also get an `Expandable` marker. Interaction emits `ValueChange<Option<Entity>>` and `TreeItemExpandChange` as requests, applied by the app or by the `tree_view_self_update` and `tree_view_expand_self_update` observers. The widget never spawns rows: a row that sets `expandable` with no container yet still emits `TreeItemExpandChange`, so the app can populate it then.
